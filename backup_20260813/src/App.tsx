...
import { SajuAnalysisResult, SavedSajuItem, SavedGunghapResult } from './types';
...
export default function App() {
  ...
  // --- 궁합 탭 전용 신규 상태 ---
  const [gunghapSubTab, setGunghapSubTab] = useState<'view' | 'history'>('view');
          ...
          {(activeTab === 'storage' || activeTab === '보관함') ? (
            <div id="view-storage" className="flex flex-col h-full justify-between bg-navy">
              ...
              <div className="bg-navy border-t border-gray-800/80 px-2 py-3 grid grid-cols-5 items-center w-full z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.4)]">
                ...
                <button
                  onClick={() => { setActiveTab('premium'); setViewMode('result'); setActiveFilter('daewoon'); }}
            ...
            <div id="view-gunghap" className="flex flex-col h-full bg-[#0a0f1d] text-white relative">
              ...
              <div className="px-5 pt-4 pb-3 border-b border-gray-800/80 bg-[#131d33] shadow-md shrink-0">
                ...
                <div className="grid grid-cols-2 gap-2 bg-[#0a0f1d] p-1.5 rounded-2xl border border-gray-800/90 shadow-inner">
                  <button
                    onClick={() => { setActiveTab('saju'); setViewMode('result'); }}
                    className="py-2.5 px-3 text-xs font-black rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 text-gray-400 hover:text-gray-200 hover:bg-gray-800/50 border border-transparent font-bold"
                  >
                    <span>☯️ 원국 분석</span>
                  </button>
                  <button
                    onClick={() => setGunghapSubTab('view')}
                    className={`py-2.5 px-3 text-xs font-black rounded-xl transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
                      gunghapSubTab === 'view'
                        ...
                        <div className="space-y-2.5">
                          {savedGunghapList.map((item) => {
                            ...
                            return (
                              ...
                              >
                                {/* 상단 Header: 상대 정보 및 보관기간 D-Day 표시 */}
                                <div className="flex items-start justify-between gap-2">
                                  <div className="space-y-1 min-w-0 flex-1">
                                    <h4 className="text-sm font-extrabold text-white flex items-center gap-1.5 truncate">
                                      <span className="text-pink-400 shrink-0">💕</span>
                                      <span className="truncate">{item.targetSajuName} 님과의 궁합분석 결과</span>
                                    </h4>
                                    <p className="text-[11.5px] text-gray-400 font-medium truncate">
                                      👤 {item.mySajuName} & {item.targetSajuName} ({item.targetGender === 'male' ? '남성' : '여성'}) · 생성일: {item.createdAt}
                                    </p>
                                  </div>

                                  {/* 남은기간 뱃지 */}
                                  <span className="text-[10.5px] font-bold px-2.5 py-1 rounded-lg bg-pink-950/30 text-pink-200 border border-pink-500/25 shrink-0 shadow-sm">
                                    ⏳ {expiryDDayText}
                                  </span>
                                </div>
                    ...
                    <div className="flex-1 overflow-y-auto px-4 pt-3 pb-6 space-y-3.5 no-scrollbar">
                      ...
                      <div className="space-y-2.5 shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                          {/* 그룹 필터 */}
                          <select
                            value={gunghapTargetSelectedGroup}
                            onChange={(e) => setGunghapTargetSelectedGroup(e.target.value)}
                            className="bg-[#131d33] border border-gray-700/80 rounded-xl px-2.5 py-2 text-xs text-amber-300 font-semibold focus:outline-none focus:border-amber-400 cursor-pointer shadow-inner flex-1 truncate"
                          >
                            {['전체', ...customGroups, '일반'].map((grp) => (
                              <option key={grp} value={grp} className="bg-[#131d33] text-gray-200">
                                {grp === '전체' ? '전체 그룹 ▾' : `${grp} ▾`}
                              </option>
                            ))}
                          </select>
                          ...
                          <div className="relative flex-1">
                            <button
                              type="button"
                              onClick={() => setIsGunghapTargetSortDropdownOpen(!isGunghapTargetSortDropdownOpen)}
          ...
          {viewMode === 'result' && (
            <div id="view-result" className="flex flex-col h-full">
              ...
              >
                ...
                <button
                  onClick={() => { setActiveTab('gunghap'); setViewMode('result'); setActiveFilter('action'); }}
                  className="flex flex-col items-center justify-center gap-1.5 text-rose-400 hover:text-rose-300 group cursor-pointer min-w-0 py-1"
                >
              ...
              >
                ...
                <div className="space-y-2.5 shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    {/* 그룹 필터 */}
                    <select
                      value={gunghapRepSelectedGroup}
                      onChange={(e) => setGunghapRepSelectedGroup(e.target.value)}
                      className="bg-[#131d33] border border-gray-700/80 rounded-xl px-2.5 py-2 text-xs text-amber-300 font-semibold focus:outline-none focus:border-amber-400 cursor-pointer shadow-inner flex-1 truncate"
                    >
                      {['전체', ...customGroups, '일반'].map((grp) => (
                        <option key={grp} value={grp} className="bg-[#131d33] text-gray-200">
                    ...
                    {/* 정렬 옵션 */}
                    <select
                      value={gunghapRepSortOption}
                      onChange={(e) => setGunghapRepSortOption(e.target.value as any)}
                      className="bg-[#131d33] border border-gray-700/80 rounded-xl px-2.5 py-2 text-xs text-amber-300 font-semibold focus:outline-none focus:border-amber-400 cursor-pointer shadow-inner flex-1 truncate"
                    >
                      <option value="latestSave" className="bg-[#131d33] text-gray-200">최근 저장순 ▾</option>
                      <option value="nameAsc" className="bg-[#131d33] text-gray-200">이름순 ▾</option>
                      <option value="birthAsc" className="bg-[#131d33] text-gray-200">생년월일순 ▾</option>
                      <option value="latestView" className="bg-[#131d33] text-gray-200">최근 검색순 ▾</option>
                    </select>
                  </div>
                </div>
