...
import { SajuInput, SajuAnalysisResult, SavedSajuItem, SavedGunghapResult, SavedWongukResult, SavedPremiumReport, UserAccount, SocialProvider, UserInquiry, FAQItem, NoticeItem } from './types';
import { getTenGodGroupForElement, calculateTenGodForChar, getJijangganInfo, getJijangganList, getPillarSinsalGilsin, calculate12Unseong, calculateSajuWonGuk, selectCelebMatch, STEMS, BRANCHES, detectSajuWonGukInteractions, DEFAULT_WON_GUK } from './data/sajuCalculations';
import { KOREA_CITIES, DEFAULT_CITY, CityInfo, getHourPillarBranch, calculateSolarTimeOffset } from './data/cityData';
import { loginWithSocial, getStoredUser, logoutUser, withdrawUser, consumeFreeBenefit, getSignupBenefitPolicy, getSignupBenefitSummary } from './services/authService';
...
export default function App() {
  ...
  const GUNGHAP_RETENTION_MS = 180 * 24 * 60 * 60 * 1000;
  ...
  // 남은 보관기간 텍스트 계산 함수 (예: 180일 후 삭제, 179일 후 삭제 ... 오늘 삭제)
  const getExpiryDDayText = (createdAtTimestamp?: number) => {
    if (!createdAtTimestamp) return '180일 후 삭제';
    const now = Date.now();
    const expireTime = createdAtTimestamp + GUNGHAP_RETENTION_MS;
  ...
  const downloadGunghapPdf = (item: SavedGunghapResult) => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          SOL AI 사주분석 시스템 · 본 리포트는 생성일로부터 180일간 이용 가능합니다.
        </div>
  ...
  const executeFullGunghapProcess = () => {
    ...
    setTimeout(() => {
      ...
      // 2. 회원가입 무료 사용 상태 기록 및 회원 계정 차감
      if (!hasUsedFirstFreeGunghap) {
        setHasUsedFirstFreeGunghap(true);
      }
      if (currentUser) {
        consumeFreeBenefit(currentUser, 'gunghap').then(updated => {
  ...
  const downloadPremiumReportPdf = (item: SavedPremiumReport) => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          SOL AI 프리미엄 명리 시스템 · 본 리포트는 구매일로부터 1년간 다시보기 및 출력을 지원합니다.
        </div>
  ...
  const startWongukAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      if (currentUser) {
        consumeFreeBenefit(currentUser, 'saju').then(updated => {
          setCurrentUser(updated);
        });
      }
  ...
  // 브라우저/모바일 캐시 자동 마이그레이션 이펙트 (초기 1회 실행)
  useEffect(() => {
    try {
  ...
  // 폼 입력 상태 (저장된 마지막 검색 이력이 있으면 해당 값으로 초기화, 없으면 기본값)
  const initialSearchedItem = useMemo(() => {
    try {
      const raw = localStorage.getItem('saju_last_searched_item');
      if (raw) return JSON.parse(raw);
    } catch (e) {}
    return null;
  }, []);

  const [name, setName] = useState(() => initialSearchedItem?.name || '김지훈');
  const [selectedGender, setSelectedGender] = useState<'male' | 'female'>(() => initialSearchedItem?.gender || 'male');
  const [year, setYear] = useState(() => initialSearchedItem?.birthYear ? String(initialSearchedItem.birthYear) : '1992');
  const [month, setMonth] = useState(() => initialSearchedItem?.birthMonth ? String(initialSearchedItem.birthMonth) : '5');
  const [day, setDay] = useState(() => initialSearchedItem?.birthDay ? String(initialSearchedItem.birthDay) : '20');
  const [calendar, setCalendar] = useState(() => initialSearchedItem?.calendar || '양력');
  const [birthHourVal, setBirthHourVal] = useState(() => initialSearchedItem?.birthHourVal !== undefined ? String(initialSearchedItem.birthHourVal) : '14');
  const [birthMinuteVal, setBirthMinuteVal] = useState(() => initialSearchedItem?.birthMinuteVal !== undefined ? String(initialSearchedItem.birthMinuteVal) : '35');
  const [jasiOption, setJasiOption] = useState<'yaja' | 'jo'>(() => initialSearchedItem?.jasiOption || 'yaja');

  // 표준시 자동 보정 상태
  const [applySolarCorrection] = useState(true);

  // 도움말 모달
  const [isHelpModalOpen, setIsHelpModalOpen] = useState(false);
  const [isUnknownTime, setIsUnknownTime] = useState(() => !!initialSearchedItem?.isUnknownTime);
  const [useYajasi, setUseYajasi] = useState(() => !!initialSearchedItem?.useYajasi);

  // 시/분 파싱 및 실시간 시간대/태양시 계산
  ...
  const numY = parseInt(year, 10) || 1992;
  const numM = parseInt(month, 10) || 5;
  const numD = parseInt(day, 10) || 20;

  const is23Hour = validHour === 23;
  const is0Hour = validHour === 0;
  const isJasiRange = !isUnknownTime && (is23Hour || is0Hour);
  ...
  }, [isUnknownTime, validHour, validMinute, applySolarCorrection, numY, numM, numD]);
  ...
  // --- 설정 및 고객지원/계정 관련 전용 상태 ---
  // 로그인 상태 (로컬 스토리지 및 서버 세션 연동)
  ...
  const showAuthToast = (msg: string) => {
    ...
    setTimeout(() => {
      setAuthToastMessage(null);
    }, 3000);
  };

  // 설정 화면 서브 뷰
  const [settingsSubView, setSettingsSubView] = useState<
    'main' | 'faq' | 'inquiry_main' | 'inquiry_create' | 'inquiry_list' | 'inquiry_detail' | 'notice_list' | 'notice_detail' | 'terms' | 'privacy' | 'service_info'
  >('main');
  ...
  // 문의 내역 및 작성 상태
  const [inquirySubTab, setInquirySubTab] = useState<'create' | 'list'>('create');
  const [selectedInquiry, setSelectedInquiry] = useState<UserInquiry | null>(null);
  const [inquiryCategory, setInquiryCategory] = useState<'서비스 이용 문의' | '결제 문의' | '오류/장애 문의' | '계정 문의' | '기타 문의'>('서비스 이용 문의');
  ...
  const [userInquiries, setUserInquiries] = useState<UserInquiry[]>(() => {
    try {
      const saved = localStorage.getItem('saju_user_inquiries');
      if (saved) return JSON.parse(saved);
      return [
        {
          id: 'inq_sample_1',
          userId: 'kakao_user_main',
          userProvider: 'kakao',
          userName: '카카오 회원',
          category: '서비스 이용 문의',
          title: '사주 분석 관련 문의',
          content: '대운과 세운의 교운기 분석에서 12운성의 작용 기준이 어떻게 반영되는지 궁금합니다.',
        ...
        },
        {
          ...
          userName: '카카오 회원',
          category: '서비스 이용 문의',
          title: '궁합 분석 저장 기간 문의',
          content: '생성된 궁합 리포트는 보관함에서 얼마나 오랫동안 열람할 수 있나요?',
          createdAt: '2026.08.12',
          status: 'answered',
          answer: {
            content: '안녕하세요, SOL 사주 고객센터입니다.\n\n생성된 사주 및 궁합 정밀 분석 리포트는 회원님의 보관함에 180일간 안전하게 보관되며, 보관 기간 내에는 언제든지 다시 열람하실 수 있습니다.\n\n추가 문의사항이 있으시면 언제든지 편하게 문의 남겨주세요. 감사합니다.',
  ...
  const handleWithdraw = async () => {
    if (currentUser) {
      await withdrawUser(currentUser);
    }
    setCurrentUser(null);
    setIsWithdrawModalOpen(false);
    setSettingsSubView('main');
    showAuthToast('회원 탈퇴가 완료되었습니다.');
  };
  ...
  const handleSubmitInquiry = (e: React.FormEvent) => {
    ...
    const updated = [newInquiry, ...userInquiries];
    setUserInquiries(updated);
    try {
      localStorage.setItem('saju_user_inquiries', JSON.stringify(updated));
    } catch (e) {}
    ...
    setTimeout(() => {
      setInquirySuccessAlert(false);
      setInquirySubTab('list');
      setSettingsSubView('inquiry_list');
    }, 1200);
  };
  ...
  const FAQ_LIST: FAQItem[] = [
    {
      id: 'faq_1',
      category: '회원가입 및 로그인',
      question: '회원가입은 어떻게 진행되나요?',
      answer: '카카오 또는 네이버 간편 로그인을 통해 별도의 복잡한 절차 없이 3초 만에 간편하게 가입하고 서비스를 이용하실 수 있습니다.'
    },
    {
      id: 'faq_2',
      category: '무료 이용 혜택',
      question: '회원가입 시 어떤 무료 혜택이 제공되나요?',
      answer: '회원가입 시 약 4,000자 분량의 상세 사주 원국 분석 2회와 약 2,000자 분량의 전문 상세 궁합 분석 1회가 무료로 제공됩니다.'
    },
    {
      id: 'faq_3',
      category: '사주 분석 이용 방법',
      question: '태어난 시간을 모르는 경우에도 분석이 가능한가요?',
      answer: '네, 가능합니다. [정보입력] 화면에서 [태어난 시간 모름]을 체크하시면 삼주(년·월·일)를 기준으로 한 정밀 분석 결과를 확인하실 수 있습니다.'
    },
    {
      id: 'faq_4',
      category: '사주 분석 이용 방법',
      question: '야자시(夜子時)와 조자시(朝子時)는 무엇인가요?',
      answer: '밤 23:00~00:00 사이의 시간을 다음 날의 일진으로 볼 것인지(조자시), 당일의 일진에 자시만 적용할 것인지(야자시) 구분하는 명리학적 기준입니다. 본 서비스는 정통 명리 기준을 충실히 반영합니다.'
    },
    {
      id: 'faq_5',
      category: '궁합 이용 방법',
      question: '궁합을 보려면 상대방 사주를 어떻게 등록하나요?',
      answer: '사주 보관함에 상대방의 사주를 저장한 후 [사주분석 > 궁합] 탭에서 [궁합 대상 선택]을 눌러 저장된 사주를 지정하시면 즉시 정밀 궁합 분석이 진행됩니다.'
    },
    {
      id: 'faq_6',
      category: '프리미엄 서비스',
      question: '프리미엄 분석은 일반 분석과 어떤 차이가 있나요?',
      answer: '최신 플래그십 AI 명리 엔진을 통해 A4 약 16~32장 분량에 달하는 대운·세운 종합 분석, 신살·십신 심층 작용력, 인생 주기별 운세 가이드를 깊이 있게 제공합니다.'
    },
    {
      id: 'faq_7',
      category: '분석 결과 저장 및 보관',
      question: '생성된 분석 리포트는 언제까지 보관되나요?',
      answer: '생성된 정밀 분석 보고서는 로그인 계정의 보관함에 180일간 안전하게 암호화되어 보관되며 언제든지 다시 열람할 수 있습니다.'
    },
    {
      id: 'faq_8',
      category: '결제 및 이용',
      question: '결제 후 분석 리포트를 다시 볼 때 추가 결제가 발생하나요?',
      answer: '아닙니다. 이미 결제 및 생성된 리포트는 보관 기간 동안 추가 비용 없이 무제한으로 다시 열람하실 수 있습니다.'
    },
    {
      id: 'faq_9',
      category: '서비스 이용 오류',
      question: '분석 화면이 정상적으로 로드되지 않을 때는 어떻게 하나요?',
      answer: '네트워크 연결 상태를 확인하신 후 브라우저 새로고침을 진행해 주세요. 지속적인 오류 발생 시 [설정 > 문의하기]를 통해 문의를 남겨주시면 신속하게 조치해 드립니다.'
    }
  ];
  ...
  const NOTICE_LIST: NoticeItem[] = [
    {
      id: 'notice_1',
      title: '서비스 업데이트 안내 (v1.0.0)',
      date: '2026.08.16',
      category: '업데이트',
      content: '안녕하세요, SOL 사주 만세력입니다.\n\n2026년 8월 16일, 고객지원 문의 기능 및 독립 계정 관리 시스템이 업데이트되었습니다.\n\n[주요 업데이트 내용]\n1. 1:1 고객지원 문의하기 및 답변 내역 확인 기능 도입\n2. 4대 주(년/월/일/시) 1:1 독립 카드 원국 UI 개편\n3. 만세력 지장간·12운성 가로 복합 행 공간 최적화\n4. 최신 플래그십 AI 심층 프리미엄 분석 리포트 연동\n\n더욱 정밀하고 신뢰할 수 있는 사주 명리 서비스를 제공하기 위해 최선을 다하겠습니다.'
    },
    {
      id: 'notice_2',
      title: '서비스 이용 및 무료 혜택 안내',
      date: '2026.08.10',
      category: '안내',
      content: 'SOL 사주를 이용해주시는 모든 회원님께 감사드립니다.\n\n회원가입 시 상세 원국 사주분석 2회 및 상세 궁합 분석 1회 무료 혜택이 자동 지급됩니다.\n\n상세 분석 보고서는 회원님의 보관함에 180일간 안전하게 보관되오니 편리하게 이용하시기 바랍니다.'
    },
    {
      id: 'notice_3',
      title: '정통 만세력 엔진 알고리즘 고도화 완료',
      date: '2026.08.01',
      category: '시스템',
      content: '한국 천문 표준시 및 출생 지역 경도 보정, 야자시/조자시 알고리즘 정밀 고도화 작업이 완료되었습니다.\n\n보다 정확한 절기 및 사주 원국 계산을 경험해 보시기 바랍니다.'
    }
  ];
  ...
  // 탭 네비게이션 이동 함수 (항상 콘텐츠 최상단 스크롤 초기화 보장)
  const navigateToTab = (tab: 'input' | 'storage' | 'gunghap' | 'premium' | 'settings') => {
    if (tab === 'input') {
      setActiveTab('saju');
      setViewMode('input');
    } else if (tab === 'storage') {
      ...
      setActiveTab('gunghap');
      setGunghapSubTab('wonguk');
    } else if (tab === 'premium') {
    ...
    setTimeout(() => {
      ...
      const scrollContainers = document.querySelectorAll(
        '.overflow-y-auto, #view-storage, #view-gunghap, #view-input, #view-result, #view-premium, #view-settings'
      );
  ...
  }, [activeTab, viewMode, gunghapSubTab, premiumSubView]);
  ...
  // 사주작용력 카드 슬라이드 인덱스 상태
  const [actionCardIndex, setActionCardIndex] = useState<number>(0);
  const actionCardContainerRef = useRef<HTMLDivElement>(null);
  ...
  // 사주 분석 결과 (localStorage 연동)
  const [sajuResult, setSajuResult] = useState<SajuAnalysisResult | null>(() => {
    try {
      const raw = localStorage.getItem('saju_last_searched_result');
      if (raw) return JSON.parse(raw);
    } catch (e) {}
    return null;
  });
  ...
  const submitSajuForm = async () => {
    ...
    const formattedTime = isUnknownTime
      ? '시간 미지정 (시간 모름)'
    ...
    const inputPayload = {
      ...
      calendarType: calendar.includes('윤달') ? 'lunar-leap' : calendar.includes('음력') ? 'lunar' : 'solar',
      isUnknownTime,
    ...
    try {
      ...
      let finalRes: SajuAnalysisResult;
      if (data.success && data.result) {
        finalRes = data.result;
      } else {
        finalRes = getFallbackResult();
      }
      setSajuResult(finalRes);

      // 마지막 검색 이력 로컬 저장 (비회원/회원 공통 연동)
      try {
        const searchedItem = {
          ...
          birthMonth: inputPayload.birthMonth,
          birthDay: inputPayload.birthDay,
          birthHourVal: String(validHour),
          birthMinuteVal: String(validMinute),
          calendar,
          isUnknownTime,
          jasiOption: isJasiRange ? jasiOption : undefined,
          useYajasi: is23Hour ? jasiOption === 'yaja' : false,
        ...
        localStorage.setItem('saju_last_searched_item', JSON.stringify(searchedItem));
        localStorage.setItem('saju_last_searched_result', JSON.stringify(finalRes));
      } catch (e) {}
    } catch {
      const finalRes = getFallbackResult();
      setSajuResult(finalRes);
      try {
        const searchedItem = {
          name: inputPayload.name,
  ...
  const hourNum = parseInt(birthHourVal, 10);
  const isJasi = !unknownTime && !isNaN(hourNum) && (hourNum === 23 || hourNum === 0);

  let displayYear = parseInt(year) || 1992;
  let displayMonth = parseInt(month) || 5;
  let displayDay = parseInt(day) || 20;

  // '야자시 미체크(또는 조자시 선택)' 시 23:00~23:59 출생자는 다음 날(명일) 일주/날짜 적용
  if (isJasi && jasiOption === 'jo' && hourNum === 23 && !unknownTime) {
  ...
  const currentFormattedTime = unknownTime
    ? '시간 미지정'
    : (birthHourVal !== '' && birthMinuteVal !== ''
        ? `${String(birthHourVal).padStart(2, '0')}:${String(birthMinuteVal).padStart(2, '0')}`
        : '14:35');
  const timeInfoStr = unknownTime ? '시간 미지정' : `${currentFormattedTime} (${hourBranchInfo?.branchName || '미시'})`;
  ...
  const handleConfirmSaveSaju = () => {
    ...
    const newItem: SavedSajuItem = {
      ...
      birthHourNum: isNaN(hourNum) ? undefined : hourNum,
      birthMinuteNum: isNaN(parseInt(birthMinuteVal, 10)) ? undefined : parseInt(birthMinuteVal, 10),
      birthCity: undefined,
      calendar: calTypeStr,
      group: '일반',
      memo: '',
  ...
  const handleLoadSavedSaju = (item: SavedSajuItem) => {
    ...
    if (item.birthHourNum !== undefined && item.birthHourNum !== null) {
      setBirthHourVal(String(item.birthHourNum));
    } else if (item.birthTime && item.birthTime.includes(':')) {
      const match = item.birthTime.match(/(\d{1,2}):(\d{2})/);
      if (match) {
        setBirthHourVal(match[1]);
        setBirthMinuteVal(match[2]);
      }
    }
    if (item.birthMinuteNum !== undefined && item.birthMinuteNum !== null) {
      setBirthMinuteVal(String(item.birthMinuteNum));
    }

    setIsUnknownTime(!!item.isUnknownTime);

    const inputPayload: SajuInput = {
      name: item.name,
      gender: item.gender,
      ...
      birthHour: item.isUnknownTime ? '시간 미지정 (시간 모름)' : (item.birthTime || '14:35'),
      birthHourNum: item.birthHourNum ?? (item.birthTime && item.birthTime.includes(':') ? parseInt(item.birthTime.split(':')[0], 10) : 14),
      birthMinuteNum: item.birthMinuteNum ?? (item.birthTime && item.birthTime.includes(':') ? parseInt(item.birthTime.split(':')[1], 10) : 35),
      birthCity: undefined,
      birthLongitude: DEFAULT_CITY.longitude,
      applySolarCorrection: true,
      jasiOption,
    ...
    const finalRes: SajuAnalysisResult = {
      matchPercentage: 92 + (inputPayload.birthDay % 7),
      celebName: fallbackCeleb.name,
      celebOccupation: fallbackCeleb.occupation,
    ...
    try {
      const searchedItem = {
        ...
        birthMonth: item.birthMonth,
        birthDay: item.birthDay,
        birthHourVal: item.birthHourNum !== undefined ? String(item.birthHourNum) : (item.birthTime?.includes(':') ? item.birthTime.split(':')[0] : '14'),
        birthMinuteVal: item.birthMinuteNum !== undefined ? String(item.birthMinuteNum) : (item.birthTime?.includes(':') ? item.birthTime.split(':')[1]?.slice(0, 2) : '35'),
  ...
  // 마지막으로 검색했던 만세력 원국 정보 복원 및 검색 이력 유무 확인 함수
  const restoreLastSearchedSaju = (): boolean => {
    // 1. 현재 메모리에 유효한 사주 결과가 이미 로드되어 있는 경우
    if (sajuResult) {
      return true;
    }

    // 2. localStorage에 마지막 검색 항목(saju_last_searched_item)이 있는 경우
    try {
      const rawItem = localStorage.getItem('saju_last_searched_item');
      if (rawItem) {
        const item = JSON.parse(rawItem);
        if (item && item.name && item.birthYear) {
          setName(item.name);
          setSelectedGender(item.gender || 'male');
          setYear(String(item.birthYear));
          setMonth(String(item.birthMonth));
          setDay(String(item.birthDay));
          if (item.calendar) setCalendar(item.calendar);
          if (item.birthHourVal !== undefined) setBirthHourVal(String(item.birthHourVal));
          if (item.birthMinuteVal !== undefined) setBirthMinuteVal(String(item.birthMinuteVal));
          setIsUnknownTime(!!item.isUnknownTime);
          if (item.jasiOption) setJasiOption(item.jasiOption);

          // 캐시된 결과 객체가 있으면 즉시 로드
          const rawResult = localStorage.getItem('saju_last_searched_result');
          if (rawResult) {
            const parsedResult = JSON.parse(rawResult);
            if (parsedResult && parsedResult.wonGuk) {
              setSajuResult(parsedResult);
              return true;
            }
          }

          // 없으면 즉시 원국 계산 후 결과 저장
          const inputPayload: SajuInput = {
            name: item.name,
            ...
            birthDay: item.birthDay,
            calendarType: item.calendar?.includes('윤달') ? 'lunar-leap' : item.calendar?.includes('음') ? 'lunar' : 'solar',
            birthHour: item.isUnknownTime ? '시간 미지정 (시간 모름)' : `${String(item.birthHourVal || 14).padStart(2, '0')}:${String(item.birthMinuteVal || 35).padStart(2, '0')}`,
            birthHourNum: parseInt(item.birthHourVal || '14', 10),
            birthMinuteNum: parseInt(item.birthMinuteVal || '35', 10),
            birthCity: undefined,
            birthLongitude: DEFAULT_CITY.longitude,
            applySolarCorrection: true,
            jasiOption: item.jasiOption,
            isUnknownTime: !!item.isUnknownTime,
            useYajasi: !!item.useYajasi,
          };
          const wonGukData = calculateSajuWonGuk(inputPayload);
          const dayStem = wonGukData.dayStem;
          const stemIdx = STEMS.findIndex(s => s.name === dayStem.name);
          ...
          const computedResult: SajuAnalysisResult = {
            matchPercentage: 92 + (inputPayload.birthDay % 7),
            celebName: fallbackCeleb.name,
            celebOccupation: fallbackCeleb.occupation,
            celebCategory: inputPayload.gender === 'male' ? '배우·가수' : '아이돌/아티스트',
            celebGender: inputPayload.gender,
            celebAgeGroup: fallbackCeleb.ageGroup,
            sajuPoints: fallbackCeleb.points,
            summary: fallbackCeleb.summary,
            dayMaster: {
          ...
          setSajuResult(computedResult);
          try {
            localStorage.setItem('saju_last_searched_result', JSON.stringify(computedResult));
          } catch (e) {}
          return true;
        }
      }
    ...
    // 3. 보관함(savedSajuList)에 저장된 사주가 있는 경우 가장 최근 항목 로드
    if (savedSajuList && savedSajuList.length > 0) {
      const sorted = [...savedSajuList].sort((a, b) => (b.lastViewedAt || 0) - (a.lastViewedAt || 0));
      const target = sorted[0];
      handleLoadSavedSaju(target);
      return true;
    }
  ...
  const handleSocialLogin = async (provider: SocialProvider) => {
    ...
    try {
      ...
      if (result.isNewUser) {
        const benefitSummary = getSignupBenefitSummary();
        if (benefitSummary) {
          showAuthToast(`🎉 환영합니다! 회원가입 혜택(${benefitSummary})이 지급되었습니다.`);
        } else {
          showAuthToast(`🎉 환영합니다! 회원가입이 완료되었습니다.`);
        }
      } else {
        showAuthToast(`✨ ${result.user.name}님, 다시 오신 것을 환영합니다!`);
      }

      // [로그인 후 이동 로직 최종 규칙]
      // 1. 설정 화면에서 로그인하는 경우 -> 설정 화면 유지
      if (activeTab === 'settings') {
        return;
      }

      // 2. 사주 검색 이력 여부 우선 판단
      const hasHistory = restoreLastSearchedSaju();
      if (hasHistory) {
        // 사주 검색 이력이 있는 사용자 -> 마지막으로 검색했던 만세력 원국 화면으로 복귀
        setActiveTab('saju');
        setViewMode('result');
      } else {
        // 사주 검색 이력이 없는 사용자 -> 정보입력 화면으로 이동
        setActiveTab('saju');
        setViewMode('input');
      }
    } catch (err) {
  ...
  // 대운 및 세운 정밀 연산 (사주분석 대운 흐름 6-카드용)
  const thisYear = new Date().getFullYear();
  const nowAge = Math.max(1, thisYear - displayYear + 1);

  // 대운 계산 (10년 단위, 대운수 3 기반 또는 기본 4단계)
  const DAEUN_LIST_BASE = [
    { startAge: 3, endAge: 12, name: '무신대운', hanja: '戊申', gan: '무', ji: '신', ganHanja: '戊', jiHanja: '申' },
    { startAge: 13, endAge: 22, name: '계유대운', hanja: '癸酉', gan: '계', ji: '유', ganHanja: '癸', jiHanja: '酉' },
    { startAge: 23, endAge: 32, name: '임신대운', hanja: '壬申', gan: '임', ji: '신', ganHanja: '壬', jiHanja: '申' },
    { startAge: 33, endAge: 42, name: '갑술대운', hanja: '甲戌', gan: '갑', ji: '술', ganHanja: '甲', jiHanja: '戌' },
    { startAge: 43, endAge: 52, name: '을해대운', hanja: '乙亥', gan: '을', ji: '해', ganHanja: '乙', jiHanja: '亥' },
    { startAge: 53, endAge: 62, name: '병자대운', hanja: '丙子', gan: '병', ji: '자', ganHanja: '丙', jiHanja: '子' },
    { startAge: 63, endAge: 72, name: '정축대운', hanja: '丁丑', gan: '정', ji: '축', ganHanja: '丁', jiHanja: '丑' },
    { startAge: 73, endAge: 82, name: '무인대운', hanja: '戊寅', gan: '무', ji: '인', ganHanja: '戊', jiHanja: '寅' },
    { startAge: 83, endAge: 92, name: '기묘대운', hanja: '己卯', gan: '기', ji: '묘', ganHanja: '己', jiHanja: '卯' }
  ];

  let currentDaeunIdx = DAEUN_LIST_BASE.findIndex(d => nowAge >= d.startAge && nowAge <= d.endAge);
  if (currentDaeunIdx < 0) currentDaeunIdx = 3; // 기본값 갑술대운

  const rawCurrentDaeun = DAEUN_LIST_BASE[currentDaeunIdx];
  const rawNextDaeun = DAEUN_LIST_BASE[Math.min(DAEUN_LIST_BASE.length - 1, currentDaeunIdx + 1)];

  const getStemElInfo = (stemName: string) => {
    const s = STEMS.find(x => x.name === stemName || x.hanja === stemName);
    if (!s) return { kor: '목(木)', hanja: '木', name: '갑' };
    return { kor: elementInfoMap[s.element]?.kor + `(${elementInfoMap[s.element]?.hanja})`, hanja: s.hanja, name: s.name };
  };

  const getBranchElInfo = (branchName: string) => {
    const b = BRANCHES.find(x => x.name === branchName || x.hanja === branchName);
    if (!b) return { kor: '토(土)', hanja: '土', name: '술' };
    return { kor: elementInfoMap[b.element]?.kor + `(${elementInfoMap[b.element]?.hanja})`, hanja: b.hanja, name: b.name };
  };

  const currentDaeun = {
    ...rawCurrentDaeun,
    ganInfo: getStemElInfo(rawCurrentDaeun.gan),
    jiInfo: getBranchElInfo(rawCurrentDaeun.ji),
    ganInteraction: `${rawCurrentDaeun.ganHanja}(${rawCurrentDaeun.gan})의 기운이 일간과 상호작용하며 새로운 비전과 추진력을 부여합니다.`,
    jiInteraction: `${rawCurrentDaeun.jiHanja}(${rawCurrentDaeun.ji})의 현실적 터전 위에서 내적 역량을 결집하고 안정적인 성취 기반을 마련합니다.`,
    coreThemeTitle: `${rawCurrentDaeun.name} 10년의 비약적 도약과 기반 구축`,
    coreThemeDesc: `자신의 강점을 명확히 인지하고 전문성을 확장하며, 사회적·경제적 지위를 확고히 다져나가는 결실의 대운 주기입니다.`
  };

  const nextDaeun = {
    ...rawNextDaeun,
    ganInfo: getStemElInfo(rawNextDaeun.gan),
    jiInfo: getBranchElInfo(rawNextDaeun.ji)
  };

  // 세운(歲運) 계산 (60갑자 연동)
  const getSewoonYearInfo = (targetYear: number) => {
    const stems60 = ['갑', '을', '병', '정', '무', '기', '경', '신', '임', '계'];
    const branches60 = ['자', '축', '인', '묘', '진', '사', '오', '미', '신', '유', '술', '해'];
    const stemHanjas = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
    const branchHanjas = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];

    const baseYear = 1984; // 1984년 갑자년
    const offset = (targetYear - baseYear) % 60;
    const normOffset = (offset + 60) % 60;
    const sIdx = normOffset % 10;
    const bIdx = normOffset % 12;

    const sName = stems60[sIdx];
    const bName = branches60[bIdx];
    const sHanja = stemHanjas[sIdx];
    const bHanja = branchHanjas[bIdx];

    const stemObj = STEMS.find(s => s.name === sName);
    const elName = stemObj ? elementInfoMap[stemObj.element]?.kor : '화';
    const elHanja = stemObj ? elementInfoMap[stemObj.element]?.hanja : '火';
    ...
    return {
      ...
      stem: sName,
      branch: bName,
      elementDesc: `${elName}(${elHanja}) 기운 주도`
    };
  };

  const currentSewoon = getSewoonYearInfo(thisYear);

  const daewoonSewoonSynergy = `${currentDaeun.name}의 중장기적인 발전 흐름 위에서, ${thisYear}년 ${currentSewoon.name}의 활동적 에너지가 기폭제가 되어 실질적인 성과 창출을 촉진합니다.`;
  ...
  return (
    <div className="flex justify-center items-center h-[100dvh] min-h-[100dvh] bg-black overflow-hidden">
      ...
      {viewMode === 'landing' && (
        <div className="w-full max-w-[412px] h-[100dvh] max-h-[100dvh] sm:h-[892px] sm:max-h-[100dvh] bg-navy flex flex-col justify-between relative overflow-hidden shadow-2xl border border-gray-800 px-7 pt-6 pb-12 rounded-none sm:rounded-3xl">
          ...
          <div className="w-full space-y-3 mb-6">
            ...
            {currentUser ? (
              <div className="flex items-center justify-between bg-navy/80 border border-gray-800 rounded-xl px-3.5 py-2 text-xs">
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className={`w-2 h-2 rounded-full ${currentUser.provider === 'kakao' ? 'bg-[#FEE500]' : 'bg-[#03C75A]'}`}></span>
                  <span className="text-gray-300 truncate">
                    <strong className="text-white">{currentUser.name}</strong> 님으로 이용 중
                  </span>
                </div>
                <button
                  type="button"
                  onClick={handleLogout}
                  className="text-[11px] text-gray-400 hover:text-rose-300 font-medium transition ml-2 shrink-0 cursor-pointer"
                >
                  로그아웃
                </button>
              </div>
              ...
              <div className="text-center">
                <button
                  ...
                  className="text-xs text-gray-400 hover:text-gold transition cursor-pointer"
                >
                  이미 계정이 있으신가요? <span className="text-white font-medium underline underline-offset-2">로그인 하기</span>
                </button>
              </div>
            )}
          </div>
        ...
        <div className="w-full max-w-[412px] h-[100dvh] max-h-[100dvh] sm:h-[892px] sm:max-h-[100dvh] bg-navy flex flex-col justify-between relative overflow-hidden shadow-2xl border border-gray-800 rounded-none sm:rounded-3xl">
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col relative">
            ...
            {(activeTab === 'storage' || activeTab === '보관함') ? (
              <div id="view-storage" className="flex flex-col h-full bg-navy">
                {/* 상단 영역: 헤더 + 뒤로가기 버튼 + 검색창 + 그룹 필터 탭 */}
                <div className="shrink-0 p-4 border-b border-gray-800/80 bg-navy/95 backdrop-blur-md">
                  {/* 헤더 타이틀 & 뒤로가기 버튼 (좌·중앙·우 균형 배치) */}
                  <div className="relative flex items-center justify-between min-h-[32px] mb-3.5 px-0.5">
                    <button
                      onClick={() => { setActiveTab('saju'); setViewMode('result'); }}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#131d33] border border-gray-700 text-gray-300 hover:text-white hover:border-gray-500 text-xs font-semibold transition cursor-pointer shrink-0 z-10"
                      title="이전 결과 화면으로 돌아가기"
                    >
                      <span>← 뒤로가기</span>
                    </button>
                    <div className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center gap-1.5 pointer-events-none">
                      <span className="text-xl leading-none">📇</span>
                      <h2 className="text-base sm:text-lg font-bold text-white tracking-tight whitespace-nowrap">사주 보관함</h2>
                    </div>
                    <span className="text-xs text-amber-400 font-medium px-2.5 py-1 rounded-full bg-amber-400/10 border border-amber-400/30 shrink-0 z-10">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-28 space-y-3.5 no-scrollbar">
                ...
                <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28 space-y-4.5 no-scrollbar">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-[11px] text-gray-400 px-1">
                      {remainingFreeWongukCount > 0 ? (
                        <span>✨ 회원가입 시 상세 원국 분석 2회 무료 제공 (약 4,000자 분량)</span>
                      ) : (
                        <div className="space-y-0.5">
                          <span className="text-amber-200/90 font-medium block">회원가입 무료 혜택을 모두 사용했습니다.</span>
                          <span className="text-[10px] text-gray-400 block">상세 원국 분석은 유료 서비스로 제공됩니다.</span>
                        </div>
                      )}
                  ...
                  <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28 space-y-4.5 no-scrollbar">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="font-bold text-xs">
                            {hasUsedFirstFreeGunghap ? '회원가입 무료 혜택을 사용했습니다.' : '회원가입 무료 상세 궁합 혜택 적용 가능'}
                          </div>
                          <div className="text-[10.5px] opacity-80 font-normal mt-0.5 whitespace-pre-line leading-relaxed">
                            {hasUsedFirstFreeGunghap
                              ? '회원가입 시 제공되는 무료 상세 궁합 혜택을 사용했습니다.\n추가 상세 궁합 분석은 유료 서비스로 이용할 수 있습니다.'
                              : '회원가입 시 약 2,000자 분량의 전문 상세 궁합 분석 리포트 1회를 무료로 제공합니다.'}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#131d33] border border-gray-800/80 rounded-2xl p-4 space-y-2 text-xs text-gray-300">
                      ...
                      <ul className="space-y-1.5 text-gray-400 text-[11.5px] list-disc list-inside leading-relaxed">
                        ...
                        <li>생성된 고급 리포트는 아래 보관함에 저장되어 생성일로부터 180일간 이용할 수 있습니다.</li>
                      </ul>
                    </div>
            ...
            <div id="view-premium" className="flex flex-col h-full bg-[#0a0f1d] text-white relative">
              ...
              {premiumSubView === 'storage' ? (
                ...
                /* === [프리미엄 상품 목록] 화면 === */
                <div className="flex-1 overflow-y-auto px-4 pt-4 pb-28 space-y-4 no-scrollbar">
                  {/* 1. 상단 간결한 프리미엄 핵심 안내 배너 */}
                  <div className="bg-gradient-to-br from-[#13233b] via-[#101b2f] to-[#0c1322] border border-cyan-500/30 rounded-2xl p-4 shadow-lg relative overflow-hidden space-y-1.5">
            ...
            /* --- 신규 전면 개편 [설정] 탭 전용 화면 --- */
            <div id="view-settings" className="flex flex-col h-full bg-[#0a0f1d] text-white relative">
              {/* 상단 타이틀 헤더 */}
              <div className="px-5 pt-4 pb-3 border-b border-gray-800/80 bg-[#131d33] shadow-md shrink-0">
                <div className="relative flex items-center justify-center min-h-[36px]">
                  <button
                  ...
                  <h2 className="text-base font-extrabold text-white tracking-tight flex items-center justify-center gap-1.5">
                    ...
                    {settingsSubView === 'faq' && <span>자주 묻는 질문 (FAQ)</span>}
                    {(settingsSubView === 'inquiry_main' || settingsSubView === 'inquiry_create' || settingsSubView === 'inquiry_list') && <span>고객지원 문의하기</span>}
                    {settingsSubView === 'inquiry_detail' && <span>문의 상세 확인</span>}
                    {settingsSubView === 'notice_list' && <span>공지사항</span>}
                    {settingsSubView === 'notice_detail' && <span>공지사항 상세</span>}
                    {settingsSubView === 'terms' && <span>이용약관</span>}
                    {settingsSubView === 'privacy' && <span>개인정보처리방침</span>}
                    {settingsSubView === 'service_info' && <span>서비스 정보</span>}
                  </h2>
                </div>
              </div>

              {/* 스크롤 콘텐츠 영역 */}
              <div className="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-28 space-y-4 no-scrollbar">

                {/* 1. 메인 설정 화면 */}
                {settingsSubView === 'main' && (
                  <div className="space-y-4">
                    {/* [ 1. 계정 영역 ] */}
                    <div className="bg-[#131d33] border border-gray-800/90 rounded-2xl p-4 space-y-3 shadow-md">
                      <div className="flex items-center justify-between border-b border-gray-800/70 pb-2.5">
                        <div className="flex items-center gap-2">
                          <span className="text-base">👤</span>
                          <h3 className="text-xs font-extrabold text-white tracking-tight">계정</h3>
                        </div>
                        {currentUser && (
                          <span className={`text-[10.5px] px-2 py-0.5 rounded-full font-bold border ${
                            currentUser.provider === 'kakao'
                              ? 'bg-[#FEE500]/15 text-[#FEE500] border-[#FEE500]/30'
                              : 'bg-[#03C75A]/15 text-[#03C75A] border-[#03C75A]/30'
                          }`}>
                            {currentUser.provider === 'kakao' ? '카카오 간편인증' : '네이버 간편인증'}
                          </span>
                        )}
                      </div>
                      ...
                      {currentUser ? (
                        <div className="space-y-3 pt-1">
                          <div className="bg-[#0a0f1d] p-3 rounded-xl border border-gray-800 space-y-1">
                            <p className="text-xs font-bold text-gray-200">
                              {currentUser.provider === 'kakao' ? '카카오 계정으로 로그인 중' : '네이버 계정으로 로그인 중'}
                            </p>
                            <p className="text-[11px] text-gray-400">
                              {currentUser.email || `${currentUser.name} 님`}
                            </p>
                          </div>
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            <button
                              type="button"
                              onClick={handleLogout}
                              className="flex-1 py-2 px-3 rounded-xl bg-gray-800/80 hover:bg-gray-700 text-gray-200 font-bold text-xs transition cursor-pointer border border-gray-700 active:scale-95 shadow-sm text-center"
                            >
                              로그아웃
                            </button>
                            <button
                              ...
                              className="py-2 px-3 rounded-xl bg-rose-950/30 hover:bg-rose-900/40 text-rose-300 font-semibold text-xs transition cursor-pointer border border-rose-800/40 active:scale-95 text-center"
                            >
                              회원탈퇴
                            </button>
                          </div>
                        ...
                        <div className="space-y-3 pt-1">
                          <div className="bg-[#0a0f1d] p-3.5 rounded-xl border border-gray-800 space-y-1.5">
                            <p className="text-xs font-bold text-gray-200">
                              로그인되어 있지 않습니다.
                            </p>
                            <p className="text-[11px] text-gray-400 leading-relaxed">
                              로그인하면 사주·궁합 분석을 저장하고 회원 혜택을 이용할 수 있습니다.
                            </p>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() => setIsLoginModalOpen(true)}
                            className="w-full flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-gradient-to-r from-gold/20 to-amber-500/20 hover:from-gold/30 hover:to-amber-500/30 border border-gold/40 text-gold font-extrabold text-xs transition cursor-pointer active:scale-95 shadow-sm"
                          >
                            <span>로그인하기</span>
                    ...
                    {/* [ 2. 고객지원 영역 ] */}
                    <div className="bg-[#131d33] border border-gray-800/90 rounded-2xl p-4 space-y-2.5 shadow-md">
                      <div className="flex items-center gap-2 border-b border-gray-800/70 pb-2.5">
                        <span className="text-base">🎧</span>
                        <h3 className="text-xs font-extrabold text-white tracking-tight">고객지원</h3>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedFaqCategory('전체');
                            setSettingsSubView('faq');
                          }}
                          className="w-full flex items-center justify-between p-3 rounded-xl bg-[#0a0f1d] border border-gray-800/90 hover:border-gray-700 text-left transition cursor-pointer group active:scale-[0.99]"
                        >
                          <div className="flex items-center gap-2.5">
                            <span className="text-sm">❓</span>
                            <div>
                              <p className="text-xs font-semibold text-gray-200 group-hover:text-white transition">자주 묻는 질문</p>
                              <p className="text-[10.5px] text-gray-400">서비스 이용, 무료 혜택, 결제 등 주요 FAQ</p>
                            </div>
                          </div>
                          <span className="text-gray-400 group-hover:text-gray-200 text-xs font-bold transition">›</span>
                        </button>
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (!currentUser) {
                              ...
                              setInquirySubTab('create');
                              setSettingsSubView('inquiry_create');
                            }
                          }}
                          className="w-full flex items-center justify-between p-3 rounded-xl bg-[#0a0f1d] border border-gray-800/90 hover:border-cyan-500/40 text-left transition cursor-pointer group active:scale-[0.99]"
                        >
                          <div className="flex items-center gap-2.5">
                            <span className="text-sm">💬</span>
                            <div>
                              <div className="flex items-center gap-1.5">
                                <p className="text-xs font-semibold text-gray-200 group-hover:text-cyan-300 transition">문의하기</p>
                                {!currentUser && (
                                  <span className="text-[9.5px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-400 border border-gray-700">
                                    로그인 필요
                                  </span>
                                )}
                              </div>
                              <p className="text-[10.5px] text-gray-400">새 문의 작성 및 내 문의 내역/답변 확인</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5">
                            {currentUser && (
                              <span className="text-[10.5px] px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 font-bold">
                                {userInquiries.filter(i => i.userId === currentUser.id).length}건
                              </span>
                            )}
                            <span className="text-gray-400 group-hover:text-gray-200 text-xs font-bold transition">›</span>
                          </div>
                        ...
                        <button
                          type="button"
                          onClick={() => setSettingsSubView('notice_list')}
                          className="w-full flex items-center justify-between p-3 rounded-xl bg-[#0a0f1d] border border-gray-800/90 hover:border-gray-700 text-left transition cursor-pointer group active:scale-[0.99]"
                        >
                          <div className="flex items-center gap-2.5">
                            <span className="text-sm">📢</span>
                            <div>
                              <p className="text-xs font-semibold text-gray-200 group-hover:text-white transition">공지사항</p>
                              <p className="text-[10.5px] text-gray-400">업데이트 소식 및 서비스 이용 안내</p>
                            </div>
                          </div>
                          <span className="text-gray-400 group-hover:text-gray-200 text-xs font-bold transition">›</span>
                        </button>
                    ...
                    <div className="bg-[#131d33] border border-gray-800/90 rounded-2xl p-4 space-y-2.5 shadow-md">
                      <div className="flex items-center gap-2 border-b border-gray-800/70 pb-2.5">
                        <span className="text-base">📜</span>
                        <h3 className="text-xs font-extrabold text-white tracking-tight">약관 및 정보</h3>
                      </div>

                      <div className="space-y-1 pt-1">
                        <button
                          type="button"
                          onClick={() => setSettingsSubView('terms')}
                          className="w-full flex items-center justify-between p-2.5 rounded-lg hover:bg-[#0a0f1d] text-left transition cursor-pointer group"
                        >
                          <span className="text-xs text-gray-300 group-hover:text-white font-medium">이용약관</span>
                          <span className="text-gray-500 group-hover:text-gray-300 text-xs font-bold">›</span>
                        </button>
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-between p-2.5 rounded-lg hover:bg-[#0a0f1d] text-left transition cursor-pointer group"
                        >
                          <span className="text-xs text-gray-300 group-hover:text-white font-medium">개인정보처리방침</span>
                          <span className="text-gray-500 group-hover:text-gray-300 text-xs font-bold">›</span>
                        </button>

                        <button
                          type="button"
                          onClick={() => setSettingsSubView('service_info')}
                          className="w-full flex items-center justify-between p-2.5 rounded-lg hover:bg-[#0a0f1d] text-left transition cursor-pointer group"
                        >
                          <span className="text-xs text-gray-300 group-hover:text-white font-medium">서비스 정보</span>
                          <span className="text-gray-500 group-hover:text-gray-300 text-xs font-bold">›</span>
                        </button>
                      </div>
                    </div>
                    ...
                    {/* [ 버전 정보 푸터 ] */}
                    <div className="pt-2 text-center text-gray-500 text-[11px] space-y-1">
                      <p className="font-semibold text-gray-400">SOL 사주 만세력</p>
                      <p>Version 1.0.0</p>
                    </div>
                  </div>
                )}
                ...
                {settingsSubView === 'faq' && (
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs">
                      {['전체', '회원가입 및 로그인', '무료 이용 혜택', '사주 분석 이용 방법', '궁합 이용 방법', '프리미엄 서비스', '분석 결과 저장 및 보관', '결제 및 이용', '서비스 이용 오류'].map(cat => (
                        <button
                          key={cat}
                          type="button"
                          onClick={() => setSelectedFaqCategory(cat)}
                          className={`px-3 py-1.5 rounded-full whitespace-nowrap text-[11px] font-bold transition cursor-pointer ${
                            selectedFaqCategory === cat
                              ? 'bg-gold text-black shadow-sm'
                              : 'bg-[#131d33] text-gray-400 hover:text-gray-200 border border-gray-800'
                          }`}
                        >
                          {cat}
                        </button>
                    ...
                    {/* FAQ 아코디언 목록 */}
                    <div className="space-y-2.5">
                      {FAQ_LIST.filter(item => selectedFaqCategory === '전체' || item.category === selectedFaqCategory).map(faq => {
                        const isOpen = expandedFaqId === faq.id;
                        return (
                          <div
                            key={faq.id}
                            className="bg-[#131d33] border border-gray-800 rounded-xl overflow-hidden transition shadow-sm"
                          >
                            <button
                              type="button"
                              onClick={() => setExpandedFaqId(isOpen ? null : faq.id)}
                              className="w-full flex items-center justify-between p-3.5 text-left cursor-pointer hover:bg-gray-800/40 transition gap-2"
                            >
                              <div className="flex items-center gap-2">
                                <span className="text-gold font-bold text-xs">Q.</span>
                                <span className="text-xs font-semibold text-gray-200">{faq.question}</span>
                              </div>
                              <span className={`text-gray-400 text-xs transition transform ${isOpen ? 'rotate-180 text-gold' : ''}`}>
                                ▼
                              </span>
                            </button>
                            {isOpen && (
                              <div className="px-3.5 pb-3.5 pt-1 text-[11.5px] text-gray-300 border-t border-gray-800/60 bg-[#0a0f1d]/60 leading-relaxed space-y-1">
                    ...
                    <div className="pt-2 text-center">
                      ...
                      >
                        원하는 질문이 없으신가요? 1:1 문의하기 →
                      </button>
                    </div>
                  </div>
                )}

                {/* 3. 문의하기 (새 문의 작성 & 내 문의 내역) 서브 뷰 */}
                {(settingsSubView === 'inquiry_create' || settingsSubView === 'inquiry_list') && (
                  <div className="space-y-4">
                    {/* 비로그인 상태일 때의 차단 화면 */}
                    {!currentUser ? (
                      <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-6 text-center space-y-4">
                        <div className="w-12 h-12 rounded-full bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center mx-auto text-xl">
                          🔒
                        </div>
                        <div className="space-y-1.5">
                          <h4 className="text-sm font-extrabold text-white">문의를 작성하려면 로그인이 필요합니다.</h4>
                          <p className="text-xs text-gray-400 leading-relaxed">
                            로그인하시면 별도의 연락처나 비밀번호 입력 없이<br />
                            안전하게 문의를 등록하고 답변을 확인하실 수 있습니다.
                          </p>
                        </div>
                        <button
                          type="button"
                          onClick={() => setIsLoginModalOpen(true)}
                          className="w-full py-2.5 rounded-xl bg-gold hover:bg-gold/90 text-black font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95"
                        >
                          로그인하기 →
                        </button>
                      </div>
                      ...
                      <div className="space-y-4">
                        {/* 상단 탭 전환: 새 문의 작성 ↔ 내 문의 내역 */}
                        <div className="grid grid-cols-2 gap-1.5 bg-[#131d33] p-1 rounded-xl border border-gray-800">
                          <button
                            type="button"
                            onClick={() => {
                              setInquirySubTab('create');
                          ...
                          >
                            <span>📋 내 문의 내역</span>
                            <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-cyan-400/20 text-cyan-300 font-extrabold">
                              {userInquiries.filter(i => i.userId === currentUser.id).length}
                            </span>
                          </button>
                        </div>
                        ...
                        {/* (1) 새 문의 작성 폼 */}
                        {inquirySubTab === 'create' && (
                          <form onSubmit={handleSubmitInquiry} className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-3.5 shadow-md">
                            {inquirySuccessAlert && (
                              <div className="p-3 bg-emerald-950/60 border border-emerald-500/40 text-emerald-200 text-xs rounded-xl font-bold flex items-center gap-2 animate-fadeIn">
                                <span>✓</span> 문의가 성공적으로 등록되었습니다. 내 문의 내역으로 이동합니다.
                              </div>
                            )}

                            {/* 문의 유형 선택 */}
                            <div className="space-y-1.5">
                              <label className="block text-xs font-bold text-gray-300">문의 유형</label>
                              <select
                                value={inquiryCategory}
                                onChange={(e) => setInquiryCategory(e.target.value as any)}
                                className="w-full bg-[#0a0f1d] border border-gray-700/80 rounded-xl px-3 py-2 text-xs text-gray-200 focus:outline-none focus:border-cyan-400 transition"
                              >
                                <option value="서비스 이용 문의">서비스 이용 문의</option>
                                <option value="결제 문의">결제 문의</option>
                                <option value="오류/장애 문의">오류/장애 문의</option>
                            ...
                            {/* 문의 제목 */}
                            <div className="space-y-1.5">
                              <label className="block text-xs font-bold text-gray-300">문의 제목</label>
                              <input
                                type="text"
                                value={inquiryTitle}
                                onChange={(e) => setInquiryTitle(e.target.value)}
                                placeholder="제목을 입력해주세요"
                                className="w-full bg-[#0a0f1d] border border-gray-700/80 rounded-xl px-3 py-2 text-xs text-gray-200 placeholder-gray-500 focus:outline-none focus:border-cyan-400 transition"
                              />
                            </div>

                            {/* 문의 내용 */}
                            <div className="space-y-1.5">
                              <label className="block text-xs font-bold text-gray-300">문의 내용</label>
                              <textarea
                                rows={5}
                                value={inquiryContent}
                                onChange={(e) => setInquiryContent(e.target.value)}
                                placeholder="문의 내용을 입력해주세요 (사주 분석, 결제, 기능 제안 등 일반적인 문의글을 자유롭게 작성할 수 있습니다)"
                                className="w-full bg-[#0a0f1d] border border-gray-700/80 rounded-xl p-3 text-xs text-gray-200 placeholder-gray-500 focus:outline-none focus:border-cyan-400 transition resize-none leading-relaxed"
                              />
                            </div>

                            <div className="text-[11px] text-gray-400 space-y-1 bg-[#0a0f1d] p-3 rounded-xl border border-gray-800/80">
                              <p className="font-semibold text-gray-300">💡 개인정보 및 보안 안내</p>
                              <p>• 문의 내역은 회원님의 로그인 계정에만 안전하게 귀속됩니다.</p>
                              <p>• 별도의 연락처나 비밀번호를 요구하지 않습니다.</p>
                            </div>

                            <button
                              type="submit"
                              className="w-full py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95"
                            >
                              문의 등록하기
                            </button>
                          </form>
                        ...
                        {inquirySubTab === 'list' && (
                          <div className="space-y-2.5">
                            {userInquiries.filter(i => i.userId === currentUser.id).length === 0 ? (
                              <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-8 text-center space-y-3">
                                <span className="text-2xl">📭</span>
                                <p className="text-xs text-gray-400">작성하신 문의 내역이 없습니다.</p>
                                <button
                                  type="button"
                                  onClick={() => {
                                    setInquirySubTab('create');
                                  ...
                                  className="text-xs text-cyan-400 font-bold underline underline-offset-4"
                                >
                                  새 문의 작성하기 →
                                </button>
                              </div>
                                ...
                                .map((inq) => (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      setSettingsSubView('inquiry_detail');
                                    }}
                                    className="bg-[#131d33] border border-gray-800/90 hover:border-cyan-500/40 rounded-xl p-3.5 space-y-2 transition cursor-pointer group shadow-sm active:scale-[0.99]"
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <span className="text-[11px] text-gray-500">{inq.createdAt}</span>
                                    </div>

                                    <div>
                                      <p className="text-xs font-bold text-gray-200 group-hover:text-cyan-300 transition">
                                        {inq.title}
                                      </p>
                                    ...
                                    <div className="flex items-center justify-between pt-1 border-t border-gray-800/60 text-[10.5px]">
                                      <span className="text-gray-400">{inq.category}</span>
                                      <span className="text-cyan-400 font-semibold group-hover:translate-x-0.5 transition">
                                        상세보기 →
                                      </span>
                                    </div>
                                  </div>
                ...
                {/* 4. 문의 상세 보기 서브 뷰 */}
                {settingsSubView === 'inquiry_detail' && selectedInquiry && (
                  <div className="space-y-4">
                    <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-4 shadow-md">
                      {/* 문의 상태 및 기본정보 */}
                      <div className="flex items-center justify-between border-b border-gray-800/70 pb-3">
                        ...
                        <span className="text-xs text-gray-400">{selectedInquiry.createdAt}</span>
                      </div>

                      {/* 문의 제목 및 내용 */}
                      <div className="space-y-2">
                        <div className="text-[11px] text-cyan-400 font-semibold">{selectedInquiry.category}</div>
                        <h4 className="text-sm font-bold text-white">{selectedInquiry.title}</h4>
                        <div className="bg-[#0a0f1d] p-3.5 rounded-xl border border-gray-800/80 text-xs text-gray-200 leading-relaxed whitespace-pre-line">
                        ...
                        {selectedInquiry.status === 'answered' && selectedInquiry.answer ? (
                          <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-3.5 space-y-2">
                            <div className="flex items-center justify-between text-[11px]">
                              <span className="font-bold text-emerald-300">SOL 사주 고객센터</span>
                              <span className="text-gray-400">{selectedInquiry.answer.answeredAt}</span>
                            </div>
                          ...
                          <div className="bg-[#0a0f1d] border border-gray-800 rounded-xl p-4 text-center space-y-1 text-xs">
                            <p className="font-semibold text-amber-300">답변 대기중</p>
                            <p className="text-[11px] text-gray-400">
                              관리자가 내용을 확인한 후 순차적으로 성실히 답변을 등록해 드립니다.
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-200 font-bold text-xs transition cursor-pointer border border-gray-700"
                    >
                      목록으로 돌아가기
                    </button>
                  </div>
                    ...
                    {NOTICE_LIST.map((notice) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setSettingsSubView('notice_detail');
                        }}
                        className="bg-[#131d33] border border-gray-800/90 hover:border-gray-700 rounded-xl p-3.5 space-y-1.5 transition cursor-pointer group shadow-sm active:scale-[0.99]"
                      >
                        <div className="flex items-center justify-between">
                        ...
                        <h4 className="text-xs font-bold text-gray-200 group-hover:text-white transition">
                          {notice.title}
                        </h4>
                      </div>
                    ))}
                ...
                {/* 6. 공지사항 상세 서브 뷰 */}
                {settingsSubView === 'notice_detail' && selectedNotice && (
                  <div className="space-y-4">
                    <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-3.5 shadow-md">
                      <div className="border-b border-gray-800/70 pb-3 space-y-1.5">
                        <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">
                          {selectedNotice.category}
                        </span>
                        <h3 className="text-sm font-bold text-white">{selectedNotice.title}</h3>
                        <p className="text-[11px] text-gray-400">{selectedNotice.date}</p>
                      </div>
                ...
                {settingsSubView === 'terms' && (
                  <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-4 shadow-md text-xs text-gray-300 leading-relaxed">
                    <h3 className="text-sm font-bold text-white border-b border-gray-800/70 pb-2">
                      서비스 이용약관
                    </h3>
                    <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1 no-scrollbar text-[11.5px]">
                      <div>
                        <h4 className="font-bold text-gold mb-1">제1조 (총칙)</h4>
                        <p className="text-gray-400">본 약관은 SOL 사주 만세력(이하 "서비스")의 이용과 관련하여 회사와 회원 간의 권리, 의무 및 책임사항을 규정함을 목적으로 합니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제2조 (서비스 이용)</h4>
                        <p className="text-gray-400">이용자는 본 약관 및 관련 법령을 준수하여 서비스를 이용하며, 만세력 및 AI 사주 분석 결과를 조회하고 보관할 수 있습니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제3조 (회원가입 및 계정)</h4>
                        <p className="text-gray-400">회원은 소셜 간편 로그인(카카오, 네이버 등)을 통해 계정을 생성할 수 있으며, 본인의 계정 정보를 성실히 관리할 책임이 있습니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제4조 (사주·궁합 분석 서비스)</h4>
                        <p className="text-gray-400">서비스에서 제공하는 사주 원국 및 궁합 분석 결과는 정통 명리학 이론과 AI 알고리즘을 기반으로 산출된 해석 자료입니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제5조 (프리미엄 서비스 및 결제)</h4>
                        <p className="text-gray-400">프리미엄 상세 및 심화 분석 서비스는 유료로 제공될 수 있으며, 결제 및 환불은 관련 전자상거래법 및 결제 정책에 따릅니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제6조 (이용자의 의무 및 면책)</h4>
                        <p className="text-gray-400">이용자는 타인의 명의를 도용하여 서비스를 이용해서는 안 되며, 분석 결과는 참고 자료로 활용되어야 합니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-gold mb-1">제7조 (약관의 변경)</h4>
                        <p className="text-gray-400">회사는 관련 법령을 위배하지 않는 범위 내에서 본 약관을 개정할 수 있으며, 공지사항을 통해 사전 고지합니다.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {settingsSubView === 'privacy' && (
                  <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-4 shadow-md text-xs text-gray-300 leading-relaxed">
                    ...
                    <div className="space-y-3 max-h-[480px] overflow-y-auto pr-1 no-scrollbar text-[11.5px]">
                      <div>
                        <h4 className="font-bold text-cyan-300 mb-1">1. 개인정보의 수집 및 이용 목적</h4>
                        <p className="text-gray-400">사주 만세력 계산, 궁합 분석, 회원 관리 및 1:1 고객지원 문의 응대를 위해 최소한의 개인정보를 수집 및 이용합니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-cyan-300 mb-1">2. 수집하는 개인정보 항목</h4>
                        <p className="text-gray-400">• 필수 항목: 간편 로그인 식별자(ID), 이메일, 생년월일시, 성별<br />• 선택 항목: 보관함 등록 사주 이름, 문의 작성 내용</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-cyan-300 mb-1">3. 보유 및 이용 기간</h4>
                        <p className="text-gray-400">회원 탈퇴 시 또는 법정 의무 보유 기간 만료 시 즉시 지체 없이 파기합니다.</p>
                      </div>
                      <div>
                        <h4 className="font-bold text-cyan-300 mb-1">4. 개인정보의 안전성 확보 조치</h4>
                        <p className="text-gray-400">모든 데이터는 암호화 통신(SSL/TLS) 및 안전한 보안 저장소를 통해 보호됩니다.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {/* 9. 서비스 정보 서브 뷰 */}
                {settingsSubView === 'service_info' && (
                  <div className="bg-[#131d33] border border-gray-800 rounded-2xl p-4 space-y-4 shadow-md text-xs text-gray-300">
                    <h3 className="text-sm font-bold text-white border-b border-gray-800/70 pb-2">
                      서비스 정보
                    </h3>
                    <div className="space-y-3">
                      <div className="flex justify-between py-1.5 border-b border-gray-800/50">
                        <span className="text-gray-400">서비스명</span>
                        <span className="font-bold text-white">SOL 사주 만세력</span>
                      </div>
                      <div className="space-y-1 py-1.5 border-b border-gray-800/50">
                        <span className="text-gray-400 block">서비스 소개</span>
                        <p className="text-gray-200 leading-relaxed text-[11.5px]">
                          사주 원국과 운의 흐름을 바탕으로 정통 명리학과 AI 기반 사주 분석 서비스를 제공합니다.
                        </p>
                      </div>
                      <div className="flex justify-between py-1.5 border-b border-gray-800/50">
                        <span className="text-gray-400">서비스 운영 정보</span>
                        <span className="text-gray-300 font-medium">SOL 명리 연구소 [추후 확정]</span>
                      </div>
                      <div className="flex justify-between py-1.5 border-b border-gray-800/50">
                        <span className="text-gray-400">고객 문의</span>
                        <span className="text-cyan-300 font-medium">설정 &gt; 고객지원 문의하기 [추후 입력]</span>
                      </div>
                      <div className="flex justify-between py-1.5">
                        <span className="text-gray-400">서비스 버전</span>
                        <span className="font-bold text-gold">Version 1.0.0</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div id="view-input" className="flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto px-5 pt-5 pb-28 space-y-5 no-scrollbar">
                ...
                <div className="bg-card border border-gray-800 rounded-2xl p-4 space-y-4 shadow-lg relative">
                  ...
                  <div className="space-y-2 pt-1 border-t border-gray-800/80">
                    ...
                    <div className="pt-1 flex items-center gap-2">
                      ...
                      />
                      <label htmlFor="chk-unknown-time" className="text-xs text-gray-300 cursor-pointer select-none">
                ...
                <button
                  ...
                  className="w-full bg-gold text-black font-bold py-3.5 rounded-xl text-xs shadow-[0_4px_15px_rgba(212,175,55,0.4)] hover:brightness-110 transition active:scale-[0.98] cursor-pointer"
                >
                  내 만세력 확인하기 →
                </button>
                ...
                <div className="pt-2 text-center">
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-200 transition-colors cursor-pointer py-1 px-2 group"
                  >
                    <span className="group-hover:underline underline-offset-4">내 보관함 바로가기</span>
                    <span className="text-[11px] group-hover:translate-x-0.5 transition-transform text-gray-400">→</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* 로딩 분석 상태 */}
          {viewMode === 'analyzing' && (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-8 my-auto px-2 py-6 sm:p-6 w-full">
              ...
              <div className="w-full max-w-full space-y-2.5 px-1 sm:px-2 flex flex-col items-center justify-center">
                <p className="text-lg sm:text-xl font-bold text-gold animate-pulse tracking-normal sm:tracking-wide leading-relaxed whitespace-nowrap text-center">
                  SOL 알고리즘이 사주를 정밀 분석 중입니다.
                </p>
                <p className="text-[11px] sm:text-xs text-amber-100/90 font-medium tracking-tighter sm:tracking-tight whitespace-nowrap text-center">
                  십성을 중심으로 오행과 원국의 주요 작용을 종합적으로 살펴보고 있습니다.
                </p>
              </div>
            </div>
          )}
              ...
              {activeFilter === 'action' ? (
                  ...
                  return (
                    <>
                      ...
                      <div className="flex-1 overflow-y-auto px-4 pt-2.5 pb-28 space-y-3 no-scrollbar">
                        ...
                        {/* 4. 작용 종류별 독립 카드 + 가로 스와이프 + 카드 위치 표시 + 실제 원국 관계 표시 */}
                        <div className="pb-4">
                            ...
                            (() => {
                              const CATEGORY_CONFIG: Record<string, {
                                ...
                                hap: {
                                  ...
                                  hanja: '合',
                                  title: '合 합',
                                  definition: '서로 다른 기운이 결합하여 조화와 연결을 만들어내는 관계입니다.',
                                  meaning: '친밀감 형성, 협상 능력, 유연한 커뮤니케이션 등에 강점이 나타날 수 있습니다.',
                                  borderColor: 'border-blue-500/35',
                                  badgeColor: 'bg-blue-500/15 text-blue-300 border-blue-500/30',
                                  textColor: 'text-blue-400',
                                  dotColor: 'bg-blue-400',
                                  softBg: 'bg-gradient-to-b from-blue-500/[0.04] to-transparent',
                                },
                                chung: {
                                  kor: '충',
                                  hanja: '沖',
                                  title: '沖 충',
                                  definition: '서로 마주 보며 부딪혀 정체된 흐름을 깨뜨리고 역동적인 변화를 이끌어내는 관계입니다.',
                                  meaning: '현실 안주를 탈피하고 빠른 환경 적응력, 과감한 결단력과 강력한 개혁 추진력에 강점이 있습니다.',
                                  borderColor: 'border-red-500/35',
                                  badgeColor: 'bg-red-500/15 text-red-300 border-red-500/30',
                                  textColor: 'text-red-400',
                                  dotColor: 'bg-red-400',
                                  softBg: 'bg-gradient-to-b from-red-500/[0.04] to-transparent',
                                },
                                hyeong: {
                                  kor: '형',
                                  hanja: '刑',
                                  title: '刑 형',
                                  definition: '불필요한 요소를 엄격하게 깎고 다듬어 완성도를 높이는 전문 조율의 관계입니다.',
                                  meaning: '사리분별이 매우 엄격하며 정밀한 검증, 전문 기술 분석, 디테일을 놓치지 않는 완벽주의에 강점이 있습니다.',
                                  borderColor: 'border-amber-500/35',
                                  badgeColor: 'bg-amber-500/15 text-amber-300 border-amber-500/30',
                                  textColor: 'text-amber-400',
                                  dotColor: 'bg-amber-400',
                                  softBg: 'bg-gradient-to-b from-amber-500/[0.04] to-transparent',
                                },
                                pa: {
                                  kor: '파',
                                  hanja: '破',
                                  title: '破 파',
                                  definition: '고착된 틀을 깨고 낡은 프로세스를 새롭게 개편하는 쇄신의 관계입니다.',
                                  meaning: '트렌드 변화에 기민하게 반응하며 기존 시스템을 업그레이드하고 혁신을 도모하는 체질 개선에 강점이 있습니다.',
                                  borderColor: 'border-purple-500/35',
                                  badgeColor: 'bg-purple-500/15 text-purple-300 border-purple-500/30',
                                  textColor: 'text-purple-400',
                                  dotColor: 'bg-purple-400',
                                  softBg: 'bg-gradient-to-b from-purple-500/[0.04] to-transparent',
                                },
                                hae: {
                                  kor: '해',
                                  hanja: '害',
                                  title: '害 해',
                                  definition: '지나치기 쉬운 미세한 오차와 리스크 요소를 선제적으로 감지하는 방어적 점검의 관계입니다.',
                                  meaning: '성급한 결정을 예방하고 숨겨진 문제점을 사전에 파악하여 위기를 철저히 방어하는 세심한 리스크 관리에 강점이 있습니다.',
                                  borderColor: 'border-orange-500/35',
                                  badgeColor: 'bg-orange-500/15 text-orange-300 border-orange-500/30',
                                  textColor: 'text-orange-400',
                                  dotColor: 'bg-orange-400',
                                  softBg: 'bg-gradient-to-b from-orange-500/[0.04] to-transparent',
                                },
                                wonjin: {
                                  kor: '원진',
                                  hanja: '怨嗔',
                                  title: '怨嗔 원진',
                                  definition: '사물의 본질과 상대방의 이면을 꿰뚫어보는 예리한 감성과 심리적 통찰의 관계입니다.',
                                  meaning: '남다른 영감과 독창적인 직관력, 예술적 감수성과 깊이 있는 심리 파악 능력에 뛰어난 강점이 나타납니다.',
                                  borderColor: 'border-cyan-500/35',
                                  badgeColor: 'bg-cyan-500/15 text-cyan-300 border-cyan-500/30',
                                  textColor: 'text-cyan-400',
                                  dotColor: 'bg-cyan-400',
                                  softBg: 'bg-gradient-to-b from-cyan-500/[0.04] to-transparent',
                                },
                              };

                              // 사용자 원국에서 실제 성립된 작용만 카드로 생성 (합 -> 충 -> 형 -> 파 -> 해 -> 원진 순서)
                              const categoriesOrder: ('hap' | 'chung' | 'hyeong' | 'pa' | 'hae' | 'wonjin')[] = ['hap', 'chung', 'hyeong', 'pa', 'hae', 'wonjin'];
                              const activeCards = categoriesOrder
                                .map(catKey => ({
                                  category: catKey,
                              ...
                              const safeIndex = Math.min(actionCardIndex, Math.max(0, totalCards - 1));

                              const scrollToIndex = (idx: number) => {
                                setActionCardIndex(idx);
                                if (actionCardContainerRef.current) {
                                  const cardEls = actionCardContainerRef.current.children;
                                  if (cardEls[idx]) {
                                    (cardEls[idx] as HTMLElement).scrollIntoView({
                                      behavior: 'smooth',
                                      inline: 'start',
                                      block: 'nearest',
                                    });
                              ...
                              return (
                                <div className="space-y-3">
                                  ...
                                  <div className="flex items-center justify-between px-1">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-xs font-bold text-gray-200">
                                        원국 작용력 ({totalCards}개 작용 발견)
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <span className="text-[11px] font-semibold text-gold bg-navy/80 px-2.5 py-0.5 rounded-full border border-gray-800 tracking-wider">
                                      ...
                                      <div className="flex items-center gap-1">
                                        <button
                                          ...
                                          className={`w-6 h-6 rounded-full flex items-center justify-center text-xs transition border ${
                                            safeIndex === 0
                                              ? 'border-gray-800 text-gray-600 bg-navy/30 cursor-not-allowed'
                                              : 'border-gray-700 text-gray-300 bg-navy hover:bg-gray-800 active:scale-95 cursor-pointer'
                                          }`}
                                          aria-label="이전 카드"
                                        >
                                          ‹
                                        ...
                                        <button
                                          onClick={() => scrollToIndex(Math.min(totalCards - 1, safeIndex + 1))}
                                  ...
                                  {/* 가로 스와이프 카드 컨테이너 (다음 카드가 살짝 보이는 peek 레이아웃) */}
                                  <div
                                    ref={actionCardContainerRef}
                                    onScroll={(e) => {
                                      const target = e.currentTarget;
                                      const scrollLeft = target.scrollLeft;
                                      const cardWidth = target.firstElementChild ? (target.firstElementChild as HTMLElement).offsetWidth + 12 : 280;
                                      const newIndex = Math.round(scrollLeft / cardWidth);
                                      if (newIndex !== actionCardIndex && newIndex >= 0 && newIndex < totalCards) {
                                        setActionCardIndex(newIndex);
                                      }
                                    }}
                                    className="flex overflow-x-auto snap-x snap-mandatory gap-3 pb-2 pt-0.5 no-scrollbar -mx-4 px-4 scroll-smooth"
                                    style={{ WebkitOverflowScrolling: 'touch' }}
                                  >
                                    {activeCards.map((card, idx) => {
                                      const { config, items, category } = card;

                                      return (
                                        <div
                                          key={category}
                                          className={`snap-start shrink-0 w-[86%] sm:w-[88%] max-w-[360px] bg-card border ${config.borderColor} rounded-2xl p-4 sm:p-5 shadow-lg relative overflow-hidden flex flex-col justify-between transition-all duration-300 ${
                                            idx === safeIndex ? 'ring-1 ring-gold/20 scale-[1.0] opacity-100' : 'opacity-85'
                                          }`}
                                        >
                                          {/* 은은한 배경 그라데이션 포인트 */}
                                          <div className={`absolute inset-0 ${config.softBg} pointer-events-none`} />
                                          ...
                                          <div className="space-y-4 relative z-10">
                                            ...
                                            <div className="flex items-center justify-between border-b border-gray-800/80 pb-3">
                                              <div className="flex items-center gap-2">
                                                ...
                                                <h3 className={`text-base sm:text-lg font-extrabold ${config.textColor} tracking-tight`}>
                                                  {config.title}
                                                </h3>
                                              </div>
                                              <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full border ${config.badgeColor}`}>
                                                {items.length}개 관계 성립
                                              </span>
                                            </div>
                                            ...
                                            <div className="space-y-2 pt-1 border-t border-gray-800/70">
                                              <span className="text-[11px] font-bold text-gray-300 flex items-center justify-between">
                                                <span>원국에서의 {config.kor} 관계</span>
                                              ...
                                              <div className="space-y-2">
                                                {items.map((rel) => {
                                                  // comboText 예시: "월주(巳) ↔ 일주(申) (사신합)" 또는 locationDesc 활용
                                                  const pairName = rel.title || rel.codeTag || '';
                                                  const relationText = rel.comboText 
                                                    ? rel.comboText.replace(/\s*\([^)]*\)$/, '') 
                                                    : rel.locationDesc;

                                                  return (
                                                    <div
                                                      key={rel.id}
                                                      className="bg-navy/60 border border-gray-800 rounded-xl p-2.5 sm:p-3 space-y-1"
                                                    >
                                                      <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-200">
                                                        <span className={`${config.textColor} text-sm font-bold`}>•</span>
                                                        <span className="leading-snug">{relationText}</span>
                                                      </div>
                                                      {pairName && (
                                                        <div className="pl-3.5 text-[11px] font-medium text-gold/90">
                                                          {pairName}
                                                        </div>
                                                      )}
                                                    </div>
                                          ...
                                          {/* 카드 하단 페이지 위치 인디케이터 */}
                                          <div className="pt-3 mt-2 flex items-center justify-between border-t border-gray-800/60 text-[10.5px] text-gray-400 relative z-10">
                                            <span>작용 카드 {idx + 1}</span>
                                  ...
                                  {/* 하단 점(dot) 인디케이터 */}
                                  {totalCards > 1 && (
                                    <div className="flex justify-center items-center gap-1.5 pt-1 pb-1">
                ...
                <>
                  ...
                  <div className="px-5 pt-4 pb-2.5 flex justify-between items-center border-b border-gray-800/80">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <h1 className="text-base font-bold text-white tracking-wide">{name || '김지훈'} 님</h1>
                        <span className="text-[11px] text-amber-400 font-medium">대운 흐름 정밀 분석</span>
                      </div>
                      <p className="text-[11px] text-gray-400">
                        {displayYear}.{formattedMonth}.{formattedDay} ({calTypeStr}) {timeInfoStr} · {genderStr}
                      </p>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 pt-2.5 pb-28 space-y-3.5 no-scrollbar">
                    ...
                    {/* 1. 필터 칩 네비게이션 */}
                    <div className="grid grid-cols-4 gap-2 w-full mt-1 mb-3">
                      <button
                        onClick={() => setActiveFilter('all')}
                        className="w-full py-2 px-0.5 rounded-full text-[12px] font-extrabold bg-[#1e293b] border-[1.5px] border-slate-400 text-[#f1f5f9] hover:border-slate-200 hover:text-white transition-all duration-200 cursor-pointer text-center flex items-center justify-center whitespace-nowrap"
                      >
                    ...
                    {/* [CARD 01] 현재 대운 안내 카드 */}
                    <div className="bg-[#131d33] border border-amber-400/35 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10.5px] font-bold text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded border border-amber-400/30">
                            CARD 01
                          </span>
                          <span className="text-xs font-bold text-gray-200">현재 대운 정보</span>
                        </div>
                        <span className="text-[11px] font-semibold text-amber-300 bg-amber-500/15 px-2.5 py-0.5 rounded-full border border-amber-400/20">
                          {nowAge}세 (만 {Math.max(0, nowAge - 1)}세)
                        </span>
                      </div>

                      <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3.5 flex items-center justify-between">
                        <div>
                          <p className="text-[11px] text-gray-400 font-medium">현재 진행 중인 10년 주기</p>
                          <div className="flex items-baseline gap-2 mt-0.5">
                            <span className="text-lg font-black text-white">{currentDaeun.name}</span>
                            <span className="text-sm font-bold text-amber-300 font-serif">({currentDaeun.hanja})</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-[11px] text-gray-400 block font-medium">적용 대운 연령</span>
                          <span className="text-sm font-extrabold text-cyan-300 font-mono">{currentDaeun.startAge}세 ~ {currentDaeun.endAge}세</span>
                        </div>
                      </div>

                      <p className="text-xs text-gray-300 leading-relaxed font-normal">
                        현재 삶의 큰 환경과 무대를 지배하는 10년 대운입니다. <span className="text-white font-medium">{currentDaeun.ganInfo.kor}({currentDaeun.ganInfo.hanja})</span>의 천간 기운과 <span className="text-white font-medium">{currentDaeun.jiInfo.kor}({currentDaeun.jiInfo.hanja})</span>의 지지 환경이 융합되어 사회적 진로와 인생 궤적에 핵심적인 변화를 주도합니다.
                      </p>
                    </div>

                    {/* [CARD 02] 현재 대운과 사주 원국의 상호작용 */}
                    <div className="bg-[#131d33] border border-blue-400/30 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10.5px] font-bold text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded border border-blue-400/30">
                            CARD 02
                          </span>
                          <span className="text-xs font-bold text-gray-200">대운 천간·지지와 원국 상호작용</span>
                        </div>
                        <span className="text-[11px] text-blue-300 font-medium">기운의 작용</span>
                      </div>

                      <div className="grid grid-cols-2 gap-2.5">
                        <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-bold text-gray-400">대운 천간</span>
                            <span className="text-xs font-extrabold text-blue-300">{currentDaeun.ganInfo.hanja} ({currentDaeun.ganInfo.kor})</span>
                          </div>
                          <p className="text-[11.5px] text-gray-300 leading-relaxed">
                            사회적 목표와 정신적 지향점입니다. {currentDaeun.ganInteraction}
                          </p>
                        </div>

                        <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-bold text-gray-400">대운 지지</span>
                            <span className="text-xs font-extrabold text-emerald-300">{currentDaeun.jiInfo.hanja} ({currentDaeun.jiInfo.kor})</span>
                          </div>
                          <p className="text-[11.5px] text-gray-300 leading-relaxed">
                            실질적 현실 기반과 활동 무대입니다. {currentDaeun.jiInteraction}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-blue-500/5 border border-blue-500/15 rounded-xl p-3">
                        <p className="text-xs text-gray-300 leading-relaxed">
                          <span className="text-blue-300 font-semibold">• 원국 결합 특징:</span> 대운의 기운이 본명 일간 및 원국 오행 구조와 맞물리며 잠재된 역량이 구체적인 결실로 발현되는 실질적 기반을 마련합니다.
                        </p>
                      </div>
                    </div>

                    {/* [CARD 03] 이번 10년의 핵심 흐름 */}
                    <div className="bg-[#131d33] border border-purple-400/30 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10.5px] font-bold text-purple-400 bg-purple-400/10 px-2 py-0.5 rounded border border-purple-400/30">
                            CARD 03
                          </span>
                          <span className="text-xs font-bold text-gray-200">10년 대운 핵심 흐름과 테마</span>
                        </div>
                        <span className="text-[11px] text-purple-300 font-medium">{currentDaeun.startAge}~{currentDaeun.endAge}세 주기</span>
                      </div>

                      <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3.5">
                        <h4 className="text-sm font-bold text-purple-200 mb-1.5 flex items-center gap-1.5">
                          <span>🌟</span>
                          <span>{currentDaeun.coreThemeTitle}</span>
                        </h4>
                        <p className="text-xs text-gray-300 leading-relaxed">
                          {currentDaeun.coreThemeDesc}
                        </p>
                      </div>

                      <div className="space-y-2">
                        <div className="flex items-start gap-2 text-xs text-gray-300">
                          <span className="text-purple-400 font-bold shrink-0">📈 발전 영역:</span>
                          <span>기존의 전문성과 실무 경험을 바탕으로 주도적인 영역을 넓히고 사회적 신뢰를 축적하는 시기입니다.</span>
                        </div>
                        <div className="flex items-start gap-2 text-xs text-gray-300">
                          <span className="text-amber-400 font-bold shrink-0">💡 주의 포인트:</span>
                          <span>급격한 확장보다는 내실 있는 체계를 다지고, 대인관계에서의 유연한 소통을 유지하는 것이 유리합니다.</span>
                        </div>
                      </div>
                    </div>

                    {/* [CARD 04] 현재 세운 안내 카드 */}
                    <div className="bg-[#131d33] border border-cyan-400/35 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10.5px] font-bold text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded border border-cyan-400/30">
                            CARD 04
                          </span>
                          <span className="text-xs font-bold text-gray-200">올해 세운(歲運) 정보</span>
                        </div>
                        <span className="text-[11px] font-bold text-cyan-300 bg-cyan-500/15 px-2.5 py-0.5 rounded-full border border-cyan-400/20">
                          {thisYear}년 운세
                        </span>
                      </div>
                      ...
                      <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3.5 flex items-center justify-between">
                        <div>
                          <p className="text-[11px] text-gray-400 font-medium">{thisYear}년 당해 연운</p>
                          <div className="flex items-baseline gap-2 mt-0.5">
                            <span className="text-lg font-black text-white">{currentSewoon.name}</span>
                            <span className="text-sm font-bold text-cyan-300 font-serif">({currentSewoon.hanja}년)</span>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-[11px] text-gray-400 block font-medium">연간 오행 속성</span>
                          <span className="text-xs font-bold text-cyan-200 bg-cyan-950/80 px-2.5 py-1 rounded-md border border-cyan-500/30 inline-block mt-0.5">
                            {currentSewoon.elementDesc}
                          </span>
                        </div>
                      </div>

                      <p className="text-xs text-gray-300 leading-relaxed font-normal">
                        올해 한 해 동안 직접적으로 체감되는 연간 운의 흐름입니다. <span className="text-white font-medium">{currentSewoon.name}</span>의 강렬한 기운이 일상 속 이벤트와 단기적 결실에 즉각적인 작용력을 미칩니다.
                      </p>
                    </div>

                    {/* [CARD 05] 대운 × 세운 상호작용 */}
                    <div className="bg-[#131d33] border border-emerald-400/30 rounded-2xl p-4 shadow-sm space-y-3 relative overflow-hidden">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-[10.5px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded border border-emerald-400/30">
                            CARD 05
                          </span>
                          <span className="text-xs font-bold text-gray-200">10년 대운 × 1년 세운 결합 분석</span>
                        </div>
                        <span className="text-[11px] text-emerald-300 font-medium">시너지 해석</span>
                      </div>

                      <div className="bg-[#0b1222]/80 border border-gray-800/80 rounded-xl p-3.5 space-y-2">
                        <div className="flex items-center justify-between border-b border-gray-800/80 pb-2">
                          <div className="flex items-center gap-1.5 text-xs font-bold text-white">
                            <span className="text-amber-400">{currentDaeun.name}</span>
                            <span className="text-gray-500">×</span>
                            <span className="text-cyan-400">{currentSewoon.name}</span>
                          </div>
                          <span className="text-[11px] font-semibold text-emerald-400">환경과 기회의 조화</span>
                        </div>
                        <p className="text-xs text-gray-300 leading-relaxed">
                          10년 대운이 삶의 든든한 배경과 토대가 되어주고, {thisYear}년 세운이 구체적인 실천과 기회의 방아쇠 역할을 수행합니다. {daewoonSewoonSynergy}
                        </p>
                      </div>

                      <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-3">
                        <p className="text-xs text-emerald-200/90 leading-relaxed">
                          <span className="font-bold text-emerald-300">💡 실전 활용 팁:</span> 대운의 큰 방향성을 잃지 않으면서, 올해 세운이 가져다주는 기회를 적극적으로 포착하여 실행으로 옮기는 전략이 최상의 성과를 냅니다.
                        </p>
                      </div>
                    </div>

                    {/* [CARD 06] 다음 대운 예고 (PREMIUM 유도) */}
                    <div className="bg-gradient-to-br from-[#181a2f] via-[#131d33] to-[#12243d] border border-cyan-500/40 rounded-2xl p-4.5 shadow-lg space-y-3.5 relative overflow-hidden">
                      <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 rounded-full blur-2xl pointer-events-none" />

                      <div className="flex items-center justify-between">
                        <span className="text-[10.5px] font-extrabold text-cyan-300 bg-cyan-950/80 px-2.5 py-0.5 rounded-full border border-cyan-400/30 flex items-center gap-1">
                      ...
                      <div className="bg-[#0b1222]/90 border border-cyan-500/20 rounded-xl p-3.5 space-y-1.5">
                        <span className="text-[11px] font-bold text-gray-400 block">다음 10년 주기</span>
                        <div className="text-sm sm:text-base font-extrabold text-white flex items-baseline gap-1.5">
                          <span className="text-cyan-300">{nextDaeun.startAge}세 ~ {nextDaeun.endAge}세</span>
                          <span className="text-gray-400 font-normal">·</span>
                          <span className="text-amber-300 font-serif">{nextDaeun.name} ({nextDaeun.hanja})</span>
                        </div>
                      </div>

                      <div className="space-y-2.5 pt-0.5">
                        <h4 className="text-xs sm:text-sm font-bold text-cyan-200 flex items-center gap-1.5">
                          <span>🔮</span>
                          <span>AI가 포착한 다음 대운의 변화</span>
                        </h4>

                        <div className="text-xs text-gray-300 space-y-2 leading-relaxed">
                          <p className="font-medium text-white/95">
                            다음 대운에서 현재와 다른 작용이 형성되는 흐름이 감지됩니다.
                          </p>
                          <p className="text-gray-300">
                            현재 대운과는 다른 기운이 들어오면서 원국과의 작용에도 새로운 변화가 나타날 수 있습니다.
                          </p>
                          <p className="text-amber-300/90 font-semibold pt-1">
                            다음 10년, 무엇이 달라질까요?
                          </p>
                        </div>
                      </div>
                      ...
                      <button
                        onClick={() => navigateToTab('premium')}
                        className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-cyan-500 via-cyan-600 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-extrabold text-xs sm:text-sm shadow-md active:scale-[0.98] transition-all cursor-pointer flex items-center justify-center gap-1.5 tracking-wide"
                      >
                        <span>프리미엄 서비스 보기</span>
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto px-4 pt-3 pb-28 space-y-3.5 no-scrollbar">
                    ...
                    {/* [ 최상단 독립 카드 1: 원국자 정보 카드 ] */}
                    <div className="bg-card border border-gray-800/90 rounded-2xl p-4 shadow-lg space-y-3 relative overflow-hidden">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] font-bold text-gray-400 tracking-wider">원국자 정보</span>
                          </div>
                          <h1 className="text-lg font-extrabold text-white mt-1 tracking-wide">
                            {name || '김지훈'} <span className="text-sm font-semibold text-gray-400">님</span>
                          </h1>
                          <p className="text-xs text-gray-300 mt-1 font-medium flex items-center gap-1.5 flex-wrap">
                            <span className="text-amber-300 font-semibold">{displayYear}년 {formattedMonth}월 {formattedDay}일</span>
                            <span className="text-gray-400">({calTypeStr})</span>
                            <span className="text-gray-400">·</span>
                            <span className="text-cyan-300 font-medium">{timeInfoStr}</span>
                            <span className="text-gray-400">·</span>
                            <span className="text-gray-300">{genderStr}</span>
                          </p>
                        </div>

                        <button
                          onClick={goBackToInput}
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-[#131d33] border border-cyan-500/40 text-cyan-300 hover:bg-cyan-500/10 active:scale-95 text-xs font-semibold shadow-sm transition cursor-pointer shrink-0"
                          title="생년월일시 정보 다시 입력"
                        >
                          <svg className="w-3.5 h-3.5 text-cyan-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                          </svg>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-black font-extrabold text-xs sm:text-sm shadow-[0_2px_12px_rgba(250,204,21,0.25)] active:scale-[0.99] transition cursor-pointer"
                      >
                      ...
                      return (
                        <div className="space-y-3.5">
                          {pillarsData.map((item) => {
                            const { type, title, period, accentBorder, badgeBg, pillar, sg, isUnknown } = item;
                            const activeSinsal = sg.sinsal.filter(s => s && s !== '-');
                            const activeGilsin = sg.gilsin.filter(g => g && g !== '-');
                            const jijangganList = getJijangganList(pillar.branch);
                            const unseong = calculate12Unseong(dayPillar.stem, pillar.branch);

                            return (
                              <div
                                key={type}
                                className={`bg-card border ${accentBorder} rounded-2xl p-4 shadow-md space-y-3 relative overflow-hidden`}
                              >
                                {/* 상단 헤더: 주 명칭 & 시기 */}
                                <div className="flex items-center justify-between border-b border-gray-800/80 pb-2.5">
                                  <div className="flex items-center gap-2">
                                    <span className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full border ${badgeBg}`}>
                                      {title}
                                    </span>
                                ...
                                {isUnknown ? (
                                  /* 태어난 시간 미입력 상태 */
                                  <div className="py-4 text-center space-y-1 bg-navy/40 rounded-xl border border-gray-800/60">
                                    <p className="text-xs text-gray-400 font-medium">태어난 시간이 입력되지 않았습니다.</p>
                                    <p className="text-[11px] text-gray-500">정보입력에서 시간을 설정하시면 시주 분석을 확인하실 수 있습니다.</p>
                                  </div>
                                ) : (
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-stretch">
                                    ...
                                    <div className="bg-[#0e1628] rounded-xl p-3.5 border border-gray-800/90 flex flex-col justify-between space-y-3">
                                      <div className="flex items-center justify-between border-b border-gray-800/60 pb-2">
                                        ...
                                        <span className="text-xs font-bold text-gray-300">
                                          {pillar.stemHanja}{pillar.branchHanja} ({pillar.stem}{pillar.branch})
                                        </span>
                                      </div>
                                      ...
                                      <div className="grid grid-cols-2 gap-2 text-center">
                                        {/* 천간 */}
                                        <div className="bg-navy/70 rounded-lg p-2 border border-gray-800/80">
                                          <span className="text-[10.5px] text-gray-400 font-medium block">천간</span>
                                          <div className={`text-lg font-extrabold mt-0.5 ${getElementTextColor(getElementFromStemBranch(pillar.stem))}`}>
                                            {pillar.stemHanja} <span className="text-xs font-bold">({pillar.stem})</span>
                                          </div>
                                          <div className={`text-[11px] font-semibold mt-0.5 ${getElementTextColor(getElementFromStemBranch(pillar.stem))}`}>
                                            {type === 'day' ? (pillar.tenGodStem || '비견 (일원)') : pillar.tenGodStem}
                                          </div>
                                        </div>

                                        {/* 지지 */}
                                        <div className="bg-navy/70 rounded-lg p-2 border border-gray-800/80">
                                          <span className="text-[10.5px] text-gray-400 font-medium block">지지</span>
                                          <div className={`text-lg font-extrabold mt-0.5 ${getElementTextColor(getElementFromStemBranch(pillar.branch))}`}>
                                            {pillar.branchHanja} <span className="text-xs font-bold">({pillar.branch})</span>
                                          </div>
                                          <div className={`text-[11px] font-semibold mt-0.5 ${getElementTextColor(getElementFromStemBranch(pillar.branch))}`}>
                                            {pillar.tenGodBranch}
                                          </div>
                                        </div>
                                      </div>
                                      ...
                                      {/* 지장간 & 12운성 가로 복합 행 */}
                                      <div className="bg-navy/50 rounded-lg px-2.5 py-1.5 border border-gray-800/60 flex items-center justify-between text-xs gap-1.5">
                                        {/* 좌측: 지장간 라벨 + 실제 값 */}
                                        <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                                          <span className="text-[10.5px] text-gray-400 font-semibold shrink-0">지장간</span>
                                          <div className="flex items-center gap-1 flex-wrap">
                                            {jijangganList.map((item, idx) => {
                                              const tenGod = calculateTenGodForChar(dayPillar.stem, item.hanja);
                                              return (
                                                <span key={idx} className="text-[11px] font-semibold text-gray-200 whitespace-nowrap">
                                                  <strong className="text-amber-300 font-bold">{item.hanja}</strong>
                                                  <span className="text-[10px] text-gray-400 font-normal">({tenGod})</span>
                                                </span>
                                              );
                                            })}
                                          </div>
                                        ...
                                        {/* 우측: 12운성 라벨 + 실제 값 */}
                                        <div className="flex items-center gap-1.5 shrink-0 pl-2 border-l border-gray-700/60">
                                          <span className="text-[10.5px] text-gray-400 font-semibold">12운성</span>
                                          <span className="bg-[#1e293b] border border-amber-500/40 text-amber-200 font-bold text-[11px] px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap">
                                            {unseong}
                                          </span>
                                        </div>
                                      </div>
                                    ...
                                    <div className="bg-[#0e1628] rounded-xl p-3.5 border border-gray-800/90 flex flex-col justify-between space-y-3">
                                      <div className="flex items-center justify-between border-b border-gray-800/60 pb-2">
                                        ...
                                        <span className="text-[10px] text-lime-300/90 font-normal">
                                          ▼ 터치시 풀이
                                        </span>
                                      </div>
                                      ...
                                      <div className="space-y-1.5">
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                          {activeSinsal.length > 0 ? (
                                            activeSinsal.map((sinsalItem, sIdx) => (
                                              <button
                                                type="button"
                                                key={`sinsal-${sIdx}`}
                                                onClick={() => handleOpenDetailModal(sinsalItem, 'sinsal')}
                                                className="inline-flex items-center justify-center bg-[#8C3B2B] hover:bg-[#a64532] text-orange-100 font-semibold text-xs px-2.5 py-1 leading-none rounded-md border border-orange-500/30 shadow-sm whitespace-nowrap cursor-pointer transition active:scale-95"
                                              >
                                                {sinsalItem}
                                              </button>
                                      ...
                                      {/* 길신 목록 */}
                                      <div className="space-y-1.5 border-t border-gray-800/40 pt-2">
                                        <div className="text-[11px] font-bold text-purple-300 flex items-center gap-1">
                                          <span>• 길신</span>
                                        </div>
                                        <div className="flex flex-wrap gap-1.5">
                                          {activeGilsin.length > 0 ? (
                                            activeGilsin.map((gilsinItem, gIdx) => (
                                              <button
                                                type="button"
                                                key={`gilsin-${gIdx}`}
                                                onClick={() => handleOpenDetailModal(gilsinItem, 'gilsin')}
                                                className="inline-flex items-center justify-center bg-[#4C205E] hover:bg-[#5f2876] text-purple-100 font-semibold text-xs px-2.5 py-1 leading-none rounded-md border border-purple-500/30 shadow-sm whitespace-nowrap cursor-pointer transition active:scale-95"
                                              >
                                                {gilsinItem}
                                              </button>
                    ...
                    {/* [ 최하단 이용 안내 카드: 무료 이용 혜택 │ PREMIUM 서비스 안내 ] */}
                    <div className="bg-card border border-gray-800/90 rounded-2xl p-4 shadow-md space-y-3 relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-stretch">
                        ...
                        {currentUser ? (
                          <div className="bg-[#0e1628] rounded-xl p-3.5 border border-gray-800/90 flex flex-col justify-between space-y-3">
                            <div className="flex items-center justify-between border-b border-gray-800/60 pb-2">
                              <div className="flex items-center gap-1.5">
                                <span className="text-base leading-none">🎁</span>
                                <h3 className="text-xs sm:text-sm font-bold text-white tracking-tight">무료 이용 혜택</h3>
                              </div>
                              <span className="text-[10.5px] px-2 py-0.5 rounded-full font-semibold bg-amber-400/15 text-amber-300 border border-amber-400/30">
                                {currentUser.provider === 'kakao' ? '카카오 회원' : '네이버 회원'}
                              </span>
                            </div>
                            ...
                            <div className="space-y-2 py-1">
                              {/* 사주 무료 횟수 */}
                              <div className="flex items-center justify-between bg-navy/70 px-2.5 py-1.5 rounded-lg border border-gray-800/80 text-xs">
                                <span className="text-gray-300 font-medium flex items-center gap-1">
                                  <span className="w-1.5 h-1.5 rounded-full bg-gold inline-block"></span>
                                  사주 무료 이용
                                </span>
                                <span className={`font-bold text-[11.5px] ${
                                  currentUser.freeWongukCount > 0 ? 'text-amber-300' : 'text-gray-400'
                                }`}>
                                  {currentUser.freeWongukCount > 0 ? `${currentUser.freeWongukCount}회 남음` : '사용 완료 (0회)'}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center justify-between bg-navy/70 px-2.5 py-1.5 rounded-lg border border-gray-800/80 text-xs">
                                ...
                                <span className={`font-bold text-[11.5px] ${
                                  currentUser.freeGunghapCount > 0 ? 'text-pink-300' : 'text-gray-400'
                                }`}>
                                  {currentUser.freeGunghapCount > 0 ? `${currentUser.freeGunghapCount}회 남음` : '사용 완료 (0회)'}
                                </span>
                              </div>

                              {/* 요약 상태 메시지 */}
                              <div className="pt-0.5 text-center">
                                {currentUser.freeWongukCount === 0 && currentUser.freeGunghapCount === 0 ? (
                                  <p className="text-[11px] text-gray-400 font-medium bg-navy/40 py-1 rounded border border-gray-800/60">
                                    무료 이용 혜택을 모두 사용했어요.
                                  </p>
                                ) : (
                                  <p className="text-[11px] text-emerald-300/90 font-medium bg-emerald-950/30 py-1 rounded border border-emerald-500/20">
                                    ✨ 사주 {currentUser.freeWongukCount}회 · 궁합 {currentUser.freeGunghapCount}회 무료 이용 가능
                                  </p>
                                )}
                              </div>
                            </div>
                            ...
                            const benefitPolicy = getSignupBenefitPolicy();
                            const benefitSummary = getSignupBenefitSummary(benefitPolicy);
                            const hasBenefit = benefitPolicy.enabled && !!benefitSummary;

                            return (
                              <div className="bg-[#0e1628] rounded-xl p-3.5 border border-gray-800/90 flex flex-col justify-between space-y-3">
                                <div className="flex items-center justify-between border-b border-gray-800/60 pb-2">
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-base leading-none">{hasBenefit ? '🎁' : '🔐'}</span>
                                    <h3 className="text-xs sm:text-sm font-bold text-white tracking-tight">
                                      {hasBenefit ? '회원가입 혜택' : '간편 로그인'}
                                    </h3>
                                  </div>
                                  <span className="text-[10.5px] px-2 py-0.5 rounded-full font-semibold bg-amber-400/15 text-amber-300 border border-amber-400/30">
                                    {hasBenefit ? '가입 즉시 지급' : '간편 시작'}
                                  </span>
                                </div>

                                <div className="space-y-2 py-1">
                                  <div className="bg-navy/70 px-2.5 py-2 rounded-lg border border-gray-800/80 text-xs space-y-1">
                                    {hasBenefit ? (
                                      <>
                                        <p className="font-bold text-amber-300 flex items-center gap-1">
                                          <span>✨</span> {benefitSummary}
                                        </p>
                                        <p className="text-[11px] text-gray-400">
                                          카카오 또는 네이버 {benefitPolicy.badgeText || '3초 간편 가입 시'} 무료 혜택이 즉시 제공됩니다.
                                        </p>
                                      </>
                                    ) : (
                                      <>
                                        <p className="font-bold text-amber-300 flex items-center gap-1">
                                          <span>✨</span> 간편하게 사주 및 궁합 보관
                                        </p>
                                        <p className="text-[11px] text-gray-400">
                                          카카오 또는 네이버 계정으로 간편하게 시작하고 나의 사주 데이터를 안전하게 보관하세요.
                                        </p>
                                      </>
                                    )}
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => setIsLoginModalOpen(true)}
                                    className="w-full py-2.5 rounded-lg bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-black font-extrabold text-xs shadow-md transition active:scale-95 text-center cursor-pointer"
                                  >
                                    회원가입 / 로그인 →
                                  </button>
                                </div>
                        ...
                        {/* [ 우측 영역: ✦ PREMIUM 서비스 안내 ] */}
                        <div className="bg-[#0e1628] rounded-xl p-3.5 border border-cyan-500/30 hover:border-cyan-400/50 flex flex-col justify-between space-y-3 transition">
                          <div className="flex items-center justify-between border-b border-gray-800/60 pb-2">
                            <div className="flex items-center gap-1.5">
                              <span className="text-cyan-400 text-sm font-extrabold">✦</span>
                              <h3 className="text-xs sm:text-sm font-extrabold text-cyan-300 tracking-wider">PREMIUM</h3>
                            </div>
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-cyan-400/15 text-cyan-300 border border-cyan-400/30">
                              최신 AI 심층 분석
                            </span>
                          </div>
                          ...
                          <div className="space-y-2.5 py-0.5">
                            ...
                            <h4 className="text-xs sm:text-sm font-bold text-white leading-snug">
                              최신 플래그십 AI로 더 깊이 분석하는 사주
                            </h4>

                            {/* 설명 문구 */}
                            <p className="text-[11px] text-gray-300 leading-relaxed font-normal">
                              최신 AI 기술과 전문적인 명리 분석을 바탕으로 나의 사주 구조와 성향, 인생의 흐름을 깊이 있게 분석합니다.
                            </p>

                            {/* 상세 / 심화 사주분석 목록 */}
                            <div className="space-y-1.5 pt-1 border-t border-gray-800/60 text-[11px] text-gray-200">
                              <div className="flex items-start gap-1.5">
                                <span className="text-cyan-400 font-bold shrink-0">•</span>
                                <span className="leading-tight">
                                  <strong className="text-cyan-300 font-semibold">상세 사주분석</strong> · 원국의 핵심 구조와 주요 작용
                                </span>
                              </div>
                              <div className="flex items-start gap-1.5">
                                <span className="text-purple-300 font-bold shrink-0">•</span>
                                <span className="leading-tight">
                                  <strong className="text-purple-300 font-semibold">심화 사주분석</strong> · 대운과 세운을 포함한 종합적인 흐름 분석
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          <button
                            type="button"
                            onClick={() => navigateToTab('premium')}
                            className="w-full flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-lg bg-gradient-to-r from-cyan-500/20 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 border border-cyan-400/40 text-cyan-300 font-extrabold text-xs transition cursor-pointer active:scale-95 shadow-sm mt-1"
                          >
                            <span>프리미엄 분석 시작하기</span>
                            <span className="text-sm leading-none">→</span>
                          </button>
                        </div>
                      </div>
      ...
      {/* 전역 하단 고정 네비게이션 바 (스크롤과 완전히 분리되어 상시 고정) */}
      <nav
        aria-label="하단 네비게이션"
        className="shrink-0 bg-[#0b1120] border-t border-gray-700/80 px-1.5 pt-2 grid grid-cols-5 items-center z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.8)] touch-manipulation select-none overscroll-none"
      ...
      >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 cursor-pointer min-w-0 py-1 transition ${
        ...
        >
          ...
          <span className={`text-[11px] font-bold tracking-tight whitespace-nowrap ${activeTab === 'saju' && viewMode === 'input' ? 'text-gold' : 'text-gray-300'}`}>
            정보입력
          </span>
        </button>
        ...
        >
          <div className={activeTab === 'storage' ? 'filter drop-shadow-[0_0_6px_rgba(212,175,55,0.5)] transform scale-105 transition duration-200' : ''}>
            <span className="text-xl leading-none">📇</span>
          </div>
          <span className={`text-[11px] font-bold tracking-tight whitespace-nowrap ${activeTab === 'storage' ? 'text-gold' : 'text-gray-300'}`}>
            보관함
          </span>
        </button>
        ...
        <button
          type="button"
          onClick={() => navigateToTab('gunghap')}
          className={`flex flex-col items-center justify-center gap-1 cursor-pointer min-w-0 py-1 transition ${
            activeTab === 'gunghap' || (activeTab === 'saju' && viewMode === 'result')
              ? 'text-gold'
              : 'text-gray-300 hover:text-white'
          }`}
        >
          <div className={activeTab === 'gunghap' || (activeTab === 'saju' && viewMode === 'result') ? 'filter drop-shadow-[0_0_8px_rgba(212,175,55,0.7)] transform scale-105 transition duration-200' : ''}>
            <span className="text-xl leading-none">☯️</span>
          </div>
          <span className={`text-[11px] font-bold tracking-tight whitespace-nowrap ${activeTab === 'gunghap' || (activeTab === 'saju' && viewMode === 'result') ? 'text-gold font-extrabold' : 'text-gray-300'}`}>
            사주분석
          </span>
        </button>
        ...
        >
          <div className={activeTab === 'premium' ? 'filter drop-shadow-[0_0_8px_rgba(34,211,238,0.6)] transform scale-105 transition duration-200' : ''}>
          ...
          <span className={`text-[11px] font-bold tracking-tight whitespace-nowrap ${activeTab === 'premium' ? 'text-cyan-400' : 'text-gray-300'}`}>
            프리미엄
          </span>
        </button>
        ...
        >
          <div className={activeTab === 'settings' ? 'filter drop-shadow-[0_0_6px_rgba(212,175,55,0.5)] transform scale-105 transition duration-200' : ''}>
          ...
          <span className={`text-[11px] font-bold tracking-tight whitespace-nowrap ${activeTab === 'settings' ? 'text-gold' : 'text-gray-300'}`}>
            설정
          </span>
        </button>
      </nav>
          ...
          {isHelpModalOpen && (
            <div id="help-modal" className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-5 z-[10000]">
              <div className="bg-[#131B2E] border border-gray-700/80 rounded-2xl p-5 w-full max-w-sm space-y-4 shadow-2xl relative max-h-[85%] overflow-y-auto no-scrollbar">
                <div className="flex justify-between items-center border-b border-gray-800 pb-3">
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-xs text-gray-200 leading-relaxed space-y-1">
                    ...
                    <p>상세 궁합을 확인하시겠습니까?</p>
                    {hasUsedFirstFreeGunghap ? (
                      <p className="text-[11px] text-rose-300 bg-rose-950/60 p-2 rounded-lg border border-rose-500/30 mt-2 leading-relaxed">
                        💳 회원가입 시 제공되는 무료 상세 궁합 혜택을 사용했습니다.<br />추가 상세 궁합 분석은 유료 서비스로 이용할 수 있습니다.
                      </p>
                    ) : (
                      <p className="text-[11px] text-emerald-300 bg-emerald-950/60 p-2 rounded-lg border border-emerald-500/30 mt-2">
                        ✨ 회원가입 무료 혜택을 사용합니다. (약 2,000자 상세 분석 리포트 제공)
                      </p>
                    )}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2.5 pt-1">
                  ...
                  <button
                    onClick={() => {
                      if (hasUsedFirstFreeGunghap) {
                        alert('회원가입 시 제공되는 무료 상세 궁합 혜택을 사용했습니다. 추가 상세 궁합 분석은 유료 서비스로 이용할 수 있습니다.');
                        return;
                      }
          ...
          {isGroupManageModalOpen && (
            <div
              className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn pointer-events-auto touch-none overscroll-contain"
          ...
          {isGunghapSelectModalOpen && gunghapSajuA && (
            <div
              className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn touch-none overscroll-contain"
              onClick={() => {
                setIsGunghapSelectModalOpen(false);
                setIsGunghapSubStorageOpen(false);
              }}
              ...
              >
                ...
                <div className="bg-navy p-3 rounded-xl border border-gray-800 space-y-1.5 text-xs text-gray-300">
                  ...
                  <p className="text-[11px] text-gray-400 leading-relaxed">
                    {remainingFreeWongukCount > 0 ? (
                      <>
                        · 회원가입 시 상세 원국 분석 2회 무료 제공 (약 4,000자 분량, 잔여 {remainingFreeWongukCount}회)<br />
                      </>
                    ) : (
                      <>
                        · 회원가입 무료 혜택을 모두 사용했습니다.<br />
                        · 상세 원국 분석은 유료 서비스로 제공됩니다.<br />
                      </>
                    )}
      ...
      {/* [ 공통 전역 팝업: 간편 로그인 / 회원가입 모달 ] */}
      {isLoginModalOpen && (
        <div
          className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fadeIn"
          onClick={() => !isLoggingIn && setIsLoginModalOpen(false)}
        >
          <div
            className="bg-[#131d33] border border-gray-700/90 rounded-3xl p-6 max-w-sm w-full shadow-2xl space-y-4 relative text-center"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              ...
              className="absolute right-4 top-4 text-gray-400 hover:text-white text-base font-bold p-1 cursor-pointer disabled:opacity-40"
            >
              ✕
            </button>

            <div className="space-y-1.5 pt-1">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-gold/20 to-amber-500/20 border border-gold/30 flex items-center justify-center mx-auto text-2xl shadow-inner">
                🔐
              </div>
              ...
              <p className="text-xs text-gray-300 leading-relaxed">
                카카오 또는 네이버 계정으로 간편하게 시작하세요.
              </p>
            </div>

            {/* 신규 가입 혜택 안내 배너 (정책 기반 동적 표시) */}
            {(() => {
              const benefitPolicy = getSignupBenefitPolicy();
              const benefitSummary = getSignupBenefitSummary(benefitPolicy);

              if (!benefitPolicy.enabled || !benefitSummary) {
                return null;
              }

              return (
                <div className="bg-amber-400/10 border border-amber-400/30 rounded-xl p-2.5 text-[11px] text-amber-200 font-medium leading-tight text-center animate-fadeIn">
                  <span>🎁 </span>
                  <strong className="text-amber-300 font-bold">{benefitPolicy.badgeText || '3초 간편 가입 시'}</strong> {benefitSummary}
                </div>
              );
            })()}
            ...
            <div className="space-y-2.5 pt-1">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-xl bg-[#FEE500] hover:bg-[#FDD835] text-[#191919] font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95 disabled:opacity-50"
              >
                {isLoggingIn ? (
                  <span className="inline-block w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin"></span>
                ) : (
                  <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 3c-5.52 0-10 3.58-10 8 0 2.82 1.83 5.3 4.6 6.69l-1.18 4.36c-.1.38.31.7.66.49l5.16-3.41c.25.02.5.03.76.03 5.52 0 10-3.58 10-8s-4.48-8-10-8z" />
                  </svg>
                )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-xl bg-[#03C75A] hover:bg-[#02b350] text-white font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95 disabled:opacity-50"
              >
                {isLoggingIn ? (
      ...
      {isWithdrawModalOpen && (
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 border-b border-gray-800 pb-3">
              <span className="text-xl">⚠️</span>
              <h3 className="text-sm font-extrabold text-white">정말 회원탈퇴 하시겠습니까?</h3>
            </div>

            <div className="space-y-2 text-xs text-gray-300 leading-relaxed">
              <p>
                탈퇴 시 계정에 연결된 <span className="text-rose-300 font-bold">1:1 문의 내역 및 무료 이용 혜택 정보</span>가 모두 초기화되며 복구할 수 없습니다.
              </p>
              <p className="text-[11px] text-gray-400">
                * 저장된 사주 보관함 데이터는 계속 안전하게 유지됩니다.
              </p>
            </div>
      ...
      {/* [ 전역 토스트 알림 메시지 ] */}
      {authToastMessage && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[110] max-w-xs w-full px-4 animate-fadeIn pointer-events-none">
          <div className="bg-[#1a233a]/95 border border-gold/50 shadow-2xl text-white text-xs font-medium py-2.5 px-4 rounded-xl text-center backdrop-blur-md flex items-center justify-center gap-1.5">
