...
import { SajuInput, SajuAnalysisResult, UserAccount, SocialProvider } from "./src/types.ts";
...
// In-Memory User & Session Store for Authentication
interface StoredUser extends UserAccount {
  sessionToken?: string;
}

const usersDb = new Map<string, StoredUser>(); // key: userId
const providerMap = new Map<string, string>(); // key: `${provider}_${providerId}` -> userId
const sessionMap = new Map<string, string>(); // key: sessionToken -> userId

// Default sample user for persistent demo if needed
const defaultKakaoId = 'kakao_user_main';
const defaultKakaoKey = 'kakao_main_account_1001';
const defaultUser: StoredUser = {
  id: defaultKakaoId,
  provider: 'kakao',
  providerId: 'main_account_1001',
  name: '카카오 회원',
  email: 'user@kakao.com',
  createdAt: '2026.08.16',
  createdAtTimestamp: 1786809600000,
  lastLoginAt: '2026.08.16 19:00',
  freeWongukCount: 2,
  freeGunghapCount: 1
};
usersDb.set(defaultKakaoId, defaultUser);
providerMap.set(defaultKakaoKey, defaultKakaoId);

let signupBenefitPolicy = {
  enabled: true,
  freeWongukCount: 2,
  freeGunghapCount: 1,
...
// [Auth API 0] Get/Update Signup Benefit Policy
app.get("/api/auth/benefit-policy", (req, res) => {
  res.json({ success: true, policy: signupBenefitPolicy });
});

app.post("/api/auth/benefit-policy", (req, res) => {
  const { enabled, freeWongukCount, freeGunghapCount, badgeText } = req.body;
  if (typeof enabled === 'boolean') signupBenefitPolicy.enabled = enabled;
  if (typeof freeWongukCount === 'number') signupBenefitPolicy.freeWongukCount = Math.max(0, freeWongukCount);
  if (typeof freeGunghapCount === 'number') signupBenefitPolicy.freeGunghapCount = Math.max(0, freeGunghapCount);
  if (typeof badgeText === 'string') signupBenefitPolicy.badgeText = badgeText;
  res.json({ success: true, policy: signupBenefitPolicy });
});

// [Auth API 1] Social Login / Registration
app.post("/api/auth/social-login", (req, res) => {
  try {
    const { provider, providerId, name, email } = req.body as {
    ...
    if (!provider || (provider !== 'kakao' && provider !== 'naver')) {
      return res.status(400).json({
        success: false,
        error: "유효하지 않은 소셜 로그인 제공자입니다 (카카오 또는 네이버만 지원)."
      });
    }

    // 소셜 고유 식별자 결정 (전달되지 않은 경우 일관된 데모 ID 생성)
    const effectiveProviderId = providerId?.trim() || `${provider}_id_demo_${provider === 'kakao' ? '778899' : '112233'}`;
    const accountLookupKey = `${provider}_${effectiveProviderId}`;
    ...
    // 기존 회원 존재 여부 확인
    const existingUserId = providerMap.get(accountLookupKey);

    if (existingUserId && usersDb.has(existingUserId)) {
      // 1. 기존 회원 로그인
      user = usersDb.get(existingUserId)!;
      user.lastLoginAt = timeStr;
      if (name && !user.name) user.name = name;
      if (email && !user.email) user.email = email;
    } else {
      // 2. 신규 회원 자동 가입
      isNewUser = true;
      const newUserId = `${provider}_${crypto.randomBytes(6).toString('hex')}`;
      const defaultName = provider === 'kakao' ? '카카오 회원' : '네이버 회원';
      const defaultEmail = provider === 'kakao' ? `${newUserId.slice(0, 10)}@kakao.com` : `${newUserId.slice(0, 10)}@naver.com`;
      ...
      user = {
        ...
        lastLoginAt: timeStr,
        // 회원가입 무료 혜택 정책에 따른 초기 횟수 지급
        freeWongukCount: signupBenefitPolicy.enabled ? signupBenefitPolicy.freeWongukCount : 0,
        freeGunghapCount: signupBenefitPolicy.enabled ? signupBenefitPolicy.freeGunghapCount : 0
      };

      usersDb.set(newUserId, user);
      providerMap.set(accountLookupKey, newUserId);
    }

    // 세션 토큰 발급
    const sessionToken = `st_${crypto.randomBytes(24).toString('hex')}`;
    sessionMap.set(sessionToken, user.id);
    ...
    return res.json({
      ...
      user: {
        ...
        freeWongukCount: user.freeWongukCount,
        freeGunghapCount: user.freeGunghapCount
      }
    });
  } catch (error) {
    console.error("Social login error:", error);
    return res.status(500).json({ success: false, error: "소셜 로그인 처리 중 오류가 발생했습니다." });
  }
});

// [Auth API 2] Get Current User Info
app.get("/api/auth/me", (req, res) => {
  try {
    ...
    if (!userId || !usersDb.has(userId)) {
      return res.status(401).json({ success: false, error: "유효하지 않거나 만료된 세션입니다." });
    }
    ...
    return res.status(500).json({ success: false, error: "사용자 정보를 조회할 수 없습니다." });
  }
});

// [Auth API 3] Update / Sync Free Benefits
app.post("/api/auth/benefits/use", (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    const { type, userId: reqUserId } = req.body as { type: 'saju' | 'gunghap'; userId?: string };

    let targetUserId = reqUserId;
    if (authHeader && authHeader.startsWith("Bearer ")) {
      const token = authHeader.split(" ")[1];
      const foundId = sessionMap.get(token);
      if (foundId) targetUserId = foundId;
    }

    if (!targetUserId || !usersDb.has(targetUserId)) {
      return res.status(400).json({ success: false, error: "대상 회원을 찾을 수 없습니다." });
    }

    const user = usersDb.get(targetUserId)!;

    if (type === 'saju') {
      if (user.freeWongukCount > 0) {
        user.freeWongukCount -= 1;
      }
    } else if (type === 'gunghap') {
      if (user.freeGunghapCount > 0) {
        user.freeGunghapCount -= 1;
      }
    }
    ...
    console.error("Benefits use error:", error);
    return res.status(500).json({ success: false, error: "혜택 차감 처리 중 오류가 발생했습니다." });
  }
});
...
app.post("/api/auth/logout", (req, res) => {
  try {
    ...
    return res.json({ success: true, message: "성공적으로 로그아웃되었습니다." });
  } catch (error) {
    return res.status(500).json({ success: false, error: "로그아웃 처리 중 오류가 발생했습니다." });
  }
});
