...
export default function App() {
  ...
  // 출생지 및 진태양시 보정 상태
  const [selectedCity, setSelectedCity] = useState<CityInfo>(DEFAULT_CITY);
  const [citySearchTerm, setCitySearchTerm] = useState('');
  const [isCityDropdownOpen, setIsCityDropdownOpen] = useState(false);
  const cityDropdownRef = useRef<HTMLDivElement>(null);
  const [applySolarCorrection, setApplySolarCorrection] = useState(true);
  ...
  }, [isUnknownTime, validHour, validMinute, applySolarCorrection, selectedCity, numY, numM, numD]);
  ...
  const filteredCities = useMemo(() => {
    if (!citySearchTerm.trim()) return KOREA_CITIES.slice(0, 15);
  ...
  const handleLoadSavedSaju = (item: SavedSajuItem) => {
    ...
    if (item.birthCity) {
      const foundCity = KOREA_CITIES.find(c => c.name === item.birthCity || c.fullName.includes(item.birthCity!));
      if (foundCity) {
    ...
    const inputPayload: SajuInput = {
      ...
      birthCity: item.birthCity || selectedCity.name,
      birthLongitude: selectedCity.longitude,
      applySolarCorrection,
      jasiOption,
                ...
                <div className="space-y-3 text-xs text-gray-300 leading-relaxed">
                  ...
                  <div className="bg-gold/10 border border-gold/30 rounded-xl p-3 space-y-1">
                    ...
                    <p className="text-[10px] text-gray-300 leading-normal">
                      SOL AI 만세력은 사용자가 입력한 생년월일시와 출생지를 바탕으로 위 3대 보정을 자동 계산하므로, 별도의 시간 계산 없이 태어난 시각 그대로 입력하시면 됩니다.
                    </p>
                  </div>
                </div>
