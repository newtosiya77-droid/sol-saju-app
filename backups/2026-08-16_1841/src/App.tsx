...
import { SajuInput, SajuAnalysisResult, SavedSajuItem, SavedGunghapResult, SavedWongukResult, SavedPremiumReport } from './types';
...
export default function App() {
  ...
  // 폼 입력 상태 (기본값: 김지훈, 1992년 5월 20일, 양력, 남성, 14:35 미시, 서울)
  ...
  const [birthHourVal, setBirthHourVal] = useState('14');
  const [birthMinuteVal, setBirthMinuteVal] = useState('35');
  ...
  // 로그인 상태 (기본값: 카카오 로그인 상태 예시 제공 또는 미로그인 전환 지원)
  const [currentUser, setCurrentUser] = useState<UserAccount | null>(() => {
    try {
      const saved = localStorage.getItem('saju_current_user');
      if (saved) return JSON.parse(saved);
      // 최초 기본값: 카카오 로그인 상태 예시
      return {
        id: 'kakao_user_main',
        provider: 'kakao',
        name: '카카오 회원',
  ...
  const handleLogin = (provider: 'kakao' | 'naver') => {
    ...
    try {
      localStorage.setItem('saju_current_user', JSON.stringify(newUser));
    } catch (e) {}
    setIsLoginModalOpen(false);
  };
  const handleSocialLogin = handleLogin;

  const handleLogout = () => {
    setCurrentUser(null);
    try {
      localStorage.removeItem('saju_current_user');
    } catch (e) {}
  };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-black">
                          ...
                          <div className="space-y-2 py-1">
                            ...
                            <div className="flex items-center justify-between bg-navy/70 px-2.5 py-1.5 rounded-lg border border-gray-800/80 text-xs">
                              ...
                              <span className={`font-bold text-[11.5px] ${
                                remainingFreeWongukCount > 0 ? 'text-amber-300' : 'text-gray-400'
                              }`}>
                                {remainingFreeWongukCount > 0 ? `${remainingFreeWongukCount}회 남음` : '사용 완료 (0회)'}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between bg-navy/70 px-2.5 py-1.5 rounded-lg border border-gray-800/80 text-xs">
                              ...
                              <span className={`font-bold text-[11.5px] ${
                                !hasUsedFirstFreeGunghap ? 'text-pink-300' : 'text-gray-400'
                              }`}>
                                {!hasUsedFirstFreeGunghap ? '1회 남음' : '사용 완료 (0회)'}
                              </span>
                            </div>

                            {/* 요약 상태 메시지 */}
                            <div className="pt-0.5 text-center">
                              {remainingFreeWongukCount === 0 && hasUsedFirstFreeGunghap ? (
                                ...
                                <p className="text-[11px] text-emerald-300/90 font-medium bg-emerald-950/30 py-1 rounded border border-emerald-500/20">
                                  ✨ 사주 {remainingFreeWongukCount}회 · 궁합 {hasUsedFirstFreeGunghap ? 0 : 1}회 무료 이용 가능
                                </p>
                              )}
                            </div>
                          </div>
      ...
      <nav
        aria-label="하단 네비게이션"
        className="shrink-0 bg-[#0b1120] border-t border-gray-700/80 px-1.5 pt-2 grid grid-cols-5 items-center z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.8)]"
      ...
      >
        ...
        >
          ...
          {isLoginModalOpen && (
            <div
              className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-[90] p-4 animate-fadeIn"
              onClick={() => setIsLoginModalOpen(false)}
            >
              <div
                className="bg-[#131d33] border border-gray-700 rounded-3xl p-6 max-w-sm w-full shadow-2xl space-y-5 relative text-center"
                onClick={(e) => e.stopPropagation()}
              >
                <button
                  type="button"
                  onClick={() => setIsLoginModalOpen(false)}
                  className="absolute right-4 top-4 text-gray-400 hover:text-white text-sm font-bold p-1 cursor-pointer"
                >
                  ✕
                </button>

                <div className="space-y-2 pt-2">
                  <div className="w-12 h-12 rounded-2xl bg-gold/15 border border-gold/30 flex items-center justify-center mx-auto text-2xl">
                    🌟
                  </div>
                  <h3 className="text-base font-extrabold text-white">간편 로그인</h3>
                  <p className="text-xs text-gray-300 leading-relaxed">
                    로그인하고 사주·궁합 데이터 보관 및<br />
                    1:1 고객지원 혜택을 이용해보세요.
                  </p>
                </div>
                ...
                <div className="space-y-2.5 pt-1">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-[#FEE500] hover:bg-[#FDD835] text-[#191919] font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-[#03C75A] hover:bg-[#02b350] text-white font-extrabold text-xs transition cursor-pointer shadow-md active:scale-95"
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
            ...
            >
              ...
              >
                ...
                <div className="space-y-2 text-xs text-gray-300 leading-relaxed">
                  <p>
                    탈퇴 시 계정에 연결된 <span className="text-rose-300 font-bold">1:1 문의 내역 및 개인화 설정</span>이 모두 삭제되며 복구할 수 없습니다.
                  </p>
                  <p className="text-[11px] text-gray-400">
                    * 로컬 기기에 저장된 기본 사주 보관함 데이터는 유지될 수 있습니다.
                  </p>
                </div>
