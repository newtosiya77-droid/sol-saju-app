...
function parseBirthTime(birthHourStr: string): { hour: number; minute: number } {
  ...
  const match = birthHourStr.match(/(\d{1,2}):(\d{1,2})/);
  if (match) {
...
export function calculateSajuWonGuk(input: SajuInput) {
  const { birthYear, birthMonth, birthDay, birthHour, calendarType, useYajasi } = input;
  const { hour, minute } = parseBirthTime(birthHour || '');
  ...
  const isJasi = (birthHour || '').includes('자시') || (birthHour || '').includes('23:30');
  ...
  // 자시(23:30~01:30) 처리 명리학 규칙:
  // - '야자시 사용' 미체크 (useYajasi === false): 전통 표준 만세력 규칙에 따라 밤 23시 이후 자시는 다음 날(명일)로 간주되어 다음 날 일주 및 다음 날 일간 기준 자시로 계산
  // - '야자시 사용' 체크 (useYajasi === true): 야자시 적용으로 원래 당일(금일) 일주를 유지하고 당일 일간 기준 자시로 계산
  if (isJasi && !useYajasi) {
